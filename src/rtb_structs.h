...
#define RTB_MINRATIFY ((height < 90000) ? 7 : 11)
#define RTB_NOTARIES_HARDCODED 180000 // DONT CHANGE Below this height notaries were hardcoded
#define RTB_MAXBLOCKS 250000 // DONT CHANGE
...
//extern std::mutex rtb_mutex;  //todo remove

struct rtb_event
{
    struct rtb_event *related;
    uint16_t len;
    int32_t height;
...
{
public:
    event(rtb_event_type t, int32_t height) : type(t), height(height) {}
...
{
    event_rewind() : event(rtb_event_type::EVENT_REWIND, 0) {}
...
{
    event_notarized() : event(rtb_event_type::EVENT_NOTARIZED, 0), notarizedheight(0), MoMdepth(0) {
        memset(this->dest, 0, sizeof(this->dest));
    }
    event_notarized(int32_t ht, const char* _dest) : event(rtb_event_type::EVENT_NOTARIZED, ht), notarizedheight(0), MoMdepth(0) {
...
struct rtb_ccdataMoM
{
    uint256 MoM;
    int32_t MoMdepth,notarized_height,height,txi;
};

struct rtb_ccdata_entry { uint256 MoM; int32_t notarized_height,kmdheight,txi; char symbol[65]; };
struct rtb_ccdatapair { int32_t notarized_height,MoMoMoffset; };

struct rtb_ccdataMoMoM
{
    uint256 MoMoM;
    int32_t kmdstarti,kmdendi,MoMoMoffset,MoMoMdepth,numpairs,len;
    struct rtb_ccdatapair *pairs;
};

struct rtb_ccdata
{
    struct rtb_ccdata *next,*prev;
    struct rtb_ccdataMoM MoMdata;
    uint32_t CCid,len;
    char symbol[65];
};
...
public:
    ...
    uint64_t redeemed;
    uint64_t shorted;
    std::list<std::shared_ptr<rtb::event>> events;
