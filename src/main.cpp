...
// Copyright (c) 2015-2020 The RTB Platform developers
...
const string strMessageMagic = "RTB Signed Message:\n";
...
bool rtb_snapshot2(std::map <std::string, CAmount> &addressAmounts)
{
...
bool rtb_dailysnapshot(int32_t height)
{
    int reorglimit = 100; 
    ...
    // Under heavy reorg attack, its possible `rtb_notarized_height` can return a height that can't be found on chain sync.
    ...
    if ( (extraoffset= height % RTB_SNAPSHOT_INTERVAL) != 0 )
    {
    ...
    else 
    {
        ...
        notarized_height = rtb_notarized_height(&prevMoMheight,&notarized_hash,&notarized_desttxid);
    ...
    if ( !rtb_snapshot2(addressAmounts) )
        return false;
    ...
    for (int32_t n = height; n > undo_height; n--) 
    {
        ...
        if ( (pindex= rtb_chainactive(n)) == 0 || rtb_blockload(block, pindex) != 0 ) 
            return false;
...
bool IsFinalTx(const CTransaction &tx, int nBlockHeight, int64_t nBlockTime)
{
    ...
    BOOST_FOREACH(const CTxIn& txin, tx.vin)
    {
        if ( !rtb_hardfork_active(nBlockTime) && txin.nSequence == 0xfffffffe &&
        ...
        else if ( rtb_hardfork_active(nBlockTime) && txin.nSequence == 0xfffffffe &&
            (
                ((int64_t)tx.nLockTime >= LOCKTIME_THRESHOLD && (int64_t)tx.nLockTime <= nBlockTime) ||
                ((int64_t)tx.nLockTime <  LOCKTIME_THRESHOLD && (int64_t)tx.nLockTime <= nBlockHeight))
            )
...
bool ContextualCheckCoinbaseTransaction(int32_t slowflag,const CBlock *block,CBlockIndex * const previndex,const CTransaction& tx, const int nHeight,int32_t validateprices)
{
    ...
    if (((uint64_t)(tx.GetValueOut()) >= ASSETCHAINS_TIMELOCKGTE) || (rtb_ac_block_subsidy(nHeight) >= ASSETCHAINS_TIMELOCKGTE))
    {
            ...
            if (tx.vout[1].scriptPubKey.GetOp2(it, op, &opretData))
            {
                if (opretData.size() > 0 && opretData.data()[0] == OPRETTYPE_TIMELOCK)
                {
                    ...
                    if (CScriptID(opretScript) == scriptHash &&
                        ...
                        rtb_block_unlocktime(nHeight) == unlocktime)
                    {
                        return(true);
                    }
                }
                      ...
                      libzcash::ProofVerifier& verifier,int32_t txIndex, int32_t numTxs)
{
    if ( chainName.isKMD() )
    {
        ...
        if ( *(int32_t *)&array[0] == 0 )
            numbanned = rtb_bannedset(&indallvouts,array,(int32_t)(sizeof(array)/sizeof(*array)));

        for (size_t j=0; j<tx.vin.size(); j++) // for every tx.vin
        {
            for (int32_t k=0; k<numbanned; k++) // go through the array of banned txids
            {
                if ( tx.vin[j].prevout.hash == array[k] && rtb_checkvout(tx.vin[j].prevout.n,k,indallvouts) != 0 ) //(tx.vin[j].prevout.n == 1 || k >= indallvouts) )
                {
    ...
    uint256 merkleroot;
    if ( ASSETCHAINS_STAKED != 0 && rtb_newStakerActive(0, tiptime) != 0 && tx.vout.size() == 2 && DecodeStakingOpRet(tx.vout[1].scriptPubKey, merkleroot) != 0 )
    {
...
bool CheckTransactionWithoutProofVerification(uint32_t tiptime,const CTransaction& tx, CValidationState &state)
{
    ...
    int32_t invalid_private_taddr=0,z_z=0,z_t=0,t_z=0,acpublic = rtb_acpublic(tiptime), current_season = getacseason(tiptime);
    /**
     * Previously:
    ...
    BOOST_FOREACH(const CTxOut& txout, tx.vout)
    {
        ...
        if ( ASSETCHAINS_PRIVATE != 0 )
        {
            ...
            if (iscoinbase == 0 && txout.nValue > 0)
            {
                ...
                if ( rtb_isnotaryvout(destaddr,tiptime) == 0 )
                {
    ...
    BOOST_FOREACH(const JSDescription& joinsplit, tx.vjoinsplit)
    {
        ...
        if ( tiptime >= RTB_SAPLING_DEADLINE )
        {
...
bool AcceptToMemoryPool(CTxMemPool& pool, CValidationState &state, const CTransaction &tx, bool fLimitFree,bool* pfMissingInputs, bool fRejectAbsurdFee, int dosLevel)
{
    ...
    if ( chainName.isKMD() && chainActive.Tip() != nullptr
            && !rtb_validate_interest(tx,chainActive.Tip()->nHeight+1, chainActive.Tip()->GetMedianTimePast() + 777))
    {
        return state.DoS(0, error("%s: rtb_validate_interest failed txid.%s", __func__, tx.GetHash().ToString()), REJECT_INVALID, "rtb-interest-invalid");
    }
...

    {
        ...
        if ( nValueOut > 777777*COIN && RTB_VALUETOOBIG(nValueOut - 777777*COIN) != 0 ) // some room for blockreward and txfees
        ...
        if ( RTB_CONNECTING <= 0 && chainActive.Tip() != 0 )
        {
            flag = 1;
            RTB_CONNECTING = (1<<30) + (int32_t)chainActive.Tip()->nHeight + 1;
        }
...
bool ReadBlockFromDisk(int32_t height,CBlock& block, const CDiskBlockPos& pos,bool checkPOW)
{
    ...
    if ( 0 && checkPOW != 0 )
    {
        rtb_block2pubkey33(pubkey33,(CBlock *)&block);
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    if ( chainName.isKMD() )
    {
        ...
        else if ( nHeight < RTB_ENDOFERA )
            return(3 * COIN);
        else if ( nHeight < 2*RTB_ENDOFERA )
            return(2 * COIN);
        else return(COIN);
    }
    else
    {
        return(rtb_ac_block_subsidy(nHeight));
    }
    /*
     // Mining slow start
    ...
    bool CheckTxInputs(const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, int nSpendHeight, const Consensus::Params& consensusParams)
    {
        ...
        for (unsigned int i = 0; i < tx.vin.size(); i++)
        {
            ...
            if (coins->IsCoinBase()) {
                ...
                {
                    uint64_t unlockTime = rtb_block_unlocktime(coins->nHeight);
...
#ifdef RTB_ENABLE_INTEREST
            if ( chainName.isKMD() && nSpendHeight > 60000 )
            {
                if ( coins->vout[prevout.n].nValue >= 10*COIN )
                {
                    int64_t interest; int32_t txheight; uint32_t locktime;
                    if ( (interest= rtb_accrued_interest(&txheight,&locktime,prevout.hash,prevout.n,0,coins->vout[prevout.n].nValue,(int32_t)nSpendHeight-1)) != 0 )
                    {
                        nValueIn += interest;
                    }
                }
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    bool fClean = true;
    //rtb_disconnect(pindex,block); does nothing?
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck,bool fCheckPOW)
{
    ...
    if ( RTB_NSPV_SUPERLITE )
        return(true);
    if ( RTB_STOPAT != 0 && pindex->nHeight > RTB_STOPAT )
        return(false);
    ...
    if ( ASSETCHAINS_NOTARY_PAY[0] != 0 && pindex->nHeight > 10 )
    {
        ...
        int32_t notarisationTx = rtb_connectblock(true,pindex,*(CBlock *)&block);  
        ...
        if ( notarisationTx == 1 )
        {
            ...
            notarypaycheque = rtb_checknotarypay((CBlock *)&block,(int32_t)pindex->nHeight);
    ...
    //if ( RTB_TESTNET_EXPIRATION != 0 && pindex->nHeight > RTB_TESTNET_EXPIRATION ) // "testnet"
    //    return(false);
    ...
    */
    ...
    if ( ASSETCHAINS_STAKED != 0 && fCheckPOW && rtb_checkPOW(blockReward+stakeTxValue-notarypaycheque,1,(CBlock *)&block,pindex->nHeight) < 0 ) 
        return state.DoS(100, error("ConnectBlock: ac_staked chain failed slow rtb_checkPOW"),REJECT_INVALID, "failed-slow_checkPOW");
    ...
    if ( chainName.isKMD() && pindex->nHeight >= RTB_NOTARIES_HEIGHT2)
        blockReward -= sum;

    if ( ASSETCHAINS_COMMISSION != 0 || ASSETCHAINS_FOUNDERS_REWARD != 0 ) //ASSETCHAINS_OVERRIDE_PUBKEY33[0] != 0 &&
    {
        uint64_t checktoshis;
        if ( (checktoshis= rtb_commission((CBlock *)&block,(int32_t)pindex->nHeight)) != 0 )
        {
    ...
    if ( block.vtx[0].GetValueOut() > blockReward+RTB_EXTRASATOSHI )
    {
        if ( !chainName.isKMD() || pindex->nHeight >= RTB_NOTARIES_HEIGHT1 || block.vtx[0].vout[0].nValue > blockReward )
        {
    ...
    //FlushStateToDisk();
    rtb_connectblock(false,pindex,*(CBlock *)&block);  // dPoW state update.
...
void static UpdateTip(CBlockIndex *pindexNew) {
    ...
    {
        int32_t longestchain = rtb_longestchain();
...
bool static DisconnectTip(CValidationState &state, bool fBare = false) {
    ...
    //if ( ASSETCHAINS_SYMBOL[0] != 0 || pindexDelete->nHeight > 1400000 )
    {
        ...
        notarizedht = rtb_notarized_height(&prevMoMheight,&notarizedhash,&txid);
    ...
    if (!fBare) {
        ...
        for (int i = 0; i < block.vtx.size(); i++)
        {
            ...
            if (tx.IsCoinBase() || (i == block.vtx.size()-1 && rtb_newStakerActive(0, pindexDelete->nTime) == 0 && rtb_isPoS((CBlock *)&block,pindexDelete->nHeight,0) != 0) || !AcceptToMemoryPool(mempool, stateDummy, tx, false, NULL))
            {
    ...
    {
        CTransaction &tx = block.vtx[i];
        //if ((i == (block.vtx.size() - 1)) && ((ASSETCHAINS_STAKED != 0 && (rtb_isPoS((CBlock *)&block) != 0))))
        if ( rtb_newStakerActive(0, pindexDelete->nTime) == 0 && i == block.vtx.size()-1 && rtb_isPoS((CBlock *)&block,pindexDelete->nHeight,0) != 0 )
        {
#ifdef ENABLE_WALLET
             // new staking tx cannot be accepted to mempool and expires in 1 block, so no need for this! :D
             if ( !GetBoolArg("-disablewallet", false) && RTB_NSPV_FULLNODE )
...
int32_t rtb_activate_sapling(CBlockIndex *pindex)
{
    uint32_t blocktime,prevtime; CBlockIndex *prev; int32_t i,transition=0,height,prevht;
    int32_t activation = 0;
    if ( pindex == 0 )
    {
        LogPrintf("rtb_activate_sapling null pindex\n");
        return(0);
    }
    height = pindex->nHeight;
    blocktime = (uint32_t)pindex->nTime;
    //LogPrintf("rtb_activate_sapling.%d starting blocktime %u cmp.%d\n",height,blocktime,blocktime > RTB_SAPLING_ACTIVATION);
    ...
    height = pindex->nHeight;
    blocktime = (uint32_t)pindex->nTime;
    //LogPrintf("starting blocktime %u cmp.%d\n",blocktime,blocktime > RTB_SAPLING_ACTIVATION);
    if ( blocktime > RTB_SAPLING_ACTIVATION ) // find the earliest transition
    {
        while ( (prev= pindex->pprev) != 0 )
        {
            prevht = prev->nHeight;
            prevtime = (uint32_t)prev->nTime;
            //LogPrintf("(%d, %u).%d -> (%d, %u).%d\n",prevht,prevtime,prevtime > RTB_SAPLING_ACTIVATION,height,blocktime,blocktime > RTB_SAPLING_ACTIVATION);
            if ( prevht+1 != height )
            {
                LogPrintf("rtb_activate_sapling: unexpected non-contiguous ht %d vs %d\n",prevht,height);
                return(0);
            }
            if ( prevtime <= RTB_SAPLING_ACTIVATION && blocktime > RTB_SAPLING_ACTIVATION )
            {
                activation = height + 60;
                LogPrintf("%s transition at %d (%d, %u) -> (%d, %u)\n",chainName.symbol().c_str(),height,prevht,prevtime,height,blocktime);
            }
            if ( prevtime < RTB_SAPLING_ACTIVATION-3600*24 )
                break;
            pindex = prev;
            height = prevht;
            blocktime = prevtime;
...
bool static ConnectTip(CValidationState &state, CBlockIndex *pindexNew, CBlock *pblock) {
    ...
    RTB_CONNECTING = (int32_t)pindexNew->nHeight;
    ...
    if ( RTB_LONGESTCHAIN != 0 && (pindexNew->nHeight == RTB_LONGESTCHAIN || pindexNew->nHeight == RTB_LONGESTCHAIN+1) )
        RTB_INSYNC = (int32_t)pindexNew->nHeight;
    else RTB_INSYNC = 0;
    //LogPrintf("connect.%d insync.%d ASSETCHAINS_SAPLING.%d\n",(int32_t)pindexNew->nHeight,RTB_INSYNC,ASSETCHAINS_SAPLING);
    /*if ( RTB_INSYNC != 0 ) //ASSETCHAINS_SYMBOL[0] == 0 &&
        rtb_broadcast(pblock,8);
    else if ( ASSETCHAINS_SYMBOL[0] != 0 )
        rtb_broadcast(pblock,4);*/
    if ( RTB_NSPV_FULLNODE )
    {
        if ( ASSETCHAINS_SAPLING <= 0 && pindexNew->nTime > RTB_SAPLING_ACTIVATION - 24*3600 )
            rtb_activate_sapling(pindexNew);
        if ( ASSETCHAINS_CC != 0 && RTB_SNAPSHOT_INTERVAL != 0 && (pindexNew->nHeight % RTB_SNAPSHOT_INTERVAL) == 0 && pindexNew->nHeight >= RTB_SNAPSHOT_INTERVAL )
        {
            uint64_t start = time(NULL);
            if ( !rtb_dailysnapshot(pindexNew->nHeight) )
            {
...
static bool ActivateBestChainStep(bool fSkipdpow, CValidationState &state, CBlockIndex *pindexMostWork, CBlock *pblock) {
    ...
    if ( RTB_REWIND != 0 )
    {
        ...
        LogPrintf(">>>>>>>>>>> rewind start ht.%d -> RTB_REWIND.%d\n",
                chainActive.Tip()->nHeight,RTB_REWIND);
        while ( RTB_REWIND > 0 && (tipindex= chainActive.Tip()) != 0 && tipindex->nHeight > RTB_REWIND )
        {
            fBlocksDisconnected = true;
        ...
        LogPrintf("reached rewind.%d, best to do: ./rtb-cli -ac_name=%s stop\n",RTB_REWIND,chainName.symbol().c_str());
#ifdef WIN32
...
CBlockIndex* AddToBlockIndex(const CBlockHeader& block)
{
    ...
    */
    ...
    {
        if ( it->second != 0 ) // vNodes.size() >= RTB_LIMITED_NETWORKSIZE, change behavior to allow rtb_ensure to work
        {
...
bool CheckBlockHeader(int32_t *futureblockp,int32_t height,CBlockIndex *pindex, const CBlockHeader& blockhdr, CValidationState& state, bool fCheckPOW)
{
    ...
    *futureblockp = 0;
    ...
    /*rtb_index2pubkey33(pubkey33,pindex,height);
                ...
                bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    if ( fCheckPOW )
    {
        ...
        if ( ASSETCHAINS_STAKED == 0 && rtb_checkPOW(0,1,(CBlock *)&block,height) < 0 ) // checks Equihash
    ...
    {
        int32_t notaryid;
        int32_t special = rtb_chosennotary(&notaryid,height,pubkey33,tiptime);
        if (notaryid > 0 || ( notaryid == 0 && height > nS5HardforkHeight ) ) {
            ...
            if (!rtb_checkopret(pblockcopy, merkleroot)) {
                LogPrintf("failed or missing merkleroot expected.%s != merkleroot.%s\n", rtb_makeopret(pblockcopy, false).ToString().c_str(), merkleroot.ToString().c_str());
    ...
    if ( ASSETCHAINS_CC != 0 ) // CC contracts might refer to transactions in the current block, from a CC spend within the same block and out of order
    {
        ...
        while ( 1 )
        {
            ...
            for (i=0; i<block.vtx.size(); i++)
            {
                ...
                if ( tx.IsCoinBase() || !tx.vjoinsplit.empty() || !tx.vShieldedSpend.empty() || (i == block.vtx.size()-1 && rtb_isPoS((CBlock *)&block,height,0) != 0) )
                    continue;
                Tx = tx;
    ...
    if ( fCheckPOW && rtb_check_deposit(height,block) < 0 )
    {
        //static uint32_t counter;
        //if ( counter++ < 100 && ASSETCHAINS_STAKED == 0 )
        //    LogPrintf("check deposit rejection\n");
        LogPrintf("CheckBlockHeader rtb_check_deposit error");
        return(false);
    }
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, CBlockIndex * const pindexPrev)
{
    ...
    if (fCheckpointsEnabled)
    {
        ...
        if ( nHeight != 0 )
        {
            ...
            if ( !rtb_checkpoint(&notarized_height,nHeight,hash) )
            {
                CBlockIndex *heightblock = chainActive[nHeight];
                if ( heightblock != 0 && heightblock->GetBlockHash() == hash )
                    return true;
...
bool ContextualCheckBlock(int32_t slowflag,const CBlock& block, CValidationState& state, CBlockIndex * const pindexPrev)
{
    ...
    const int32_t txheight = nHeight == 0 ? rtb_block2height((CBlock *)&block) : nHeight;
    ...
    for (uint32_t i = 0; i < block.vtx.size(); i++) {
        ...
        // Interest validation
        if (!rtb_validate_interest(tx, txheight, cmptime))
        {
            LogPrintf("validate intrest failed for txnum.%i tx.%s\n", i, tx.ToString());
            return state.DoS(0, error("%s: rtb_validate_interest failed", __func__), REJECT_INVALID, "rtb-interest-invalid");
        }
...
bool AcceptBlockHeader(int32_t *futureblockp,const CBlockHeader& block, CValidationState& state, CBlockIndex** ppindex)
{
    ...
    if (miSelf != mapBlockIndex.end())
    {
        ...
        {
            if ( ASSETCHAINS_CC == 0 )//&& (ASSETCHAINS_PRIVATE == 0 || RTB_INSYNC >= Params().GetConsensus().vUpgrades[Consensus::UPGRADE_SAPLING].nActivationHeight) )
        ...
        /*if ( pindex != 0 && hash == rtb_requestedhash )
        {
            LogPrintf("AddToBlockIndex A rtb_requestedhash %s\n",rtb_requestedhash.ToString().c_str());
            memset(&rtb_requestedhash,0,sizeof(rtb_requestedhash));
            rtb_requestedcount = 0;
        }*/
    ...
    /*if ( pindex != 0 && hash == rtb_requestedhash )
    {
        LogPrintf("AddToBlockIndex rtb_requestedhash %s\n",rtb_requestedhash.ToString().c_str());
        memset(&rtb_requestedhash,0,sizeof(rtb_requestedhash));
        rtb_requestedcount = 0;
    }*/
    return true;
...
CBlockIndex *rtb_ensure(CBlock *pblock, uint256 hash)
{
...
CBlockIndex *oldrtb_ensure(CBlock *pblock, uint256 hash)
{
    CBlockIndex *pindex=0,*previndex=0;
    if ( (pindex = rtb_getblockindex(hash)) == 0 )
    {
        pindex = new CBlockIndex();
        if (!pindex)
            throw runtime_error("rtb_ensure: new CBlockIndex failed");
    ...
    if ( miSelf == mapBlockIndex.end() )
    {
        LogPrintf("rtb_ensure unexpected missing hash %s\n",hash.ToString().c_str());
        return(0);
    }
    if ( miSelf->second == 0 ) // create pindex so first Accept block doesnt fail
    {
        ...
        if ( pindex != 0 )
        {
            miSelf->second = pindex;
            LogPrintf("Block header %s is already known, but without pindex -> ensured %p\n",hash.ToString().c_str(),miSelf->second);
        } else LogPrintf("rtb_ensure unexpected null pindex\n");
    }
    /*if ( hash != Params().GetConsensus().hashGenesisBlock )
        {
            miSelf = mapBlockIndex.find(pblock->hashPrevBlock);
            ...
            if ( (miSelf= mapBlockIndex.find(pblock->hashPrevBlock)) != mapBlockIndex.end() )
            {
                if ( miSelf->second == 0 ) // create pindex so first Accept block doesnt fail
                {
                    ...
                    if ( previndex != 0 )
                    {
                        ...
                        LogPrintf("autocreate previndex %s\n",pblock->hashPrevBlock.ToString().c_str());
                    } else LogPrintf("rtb_ensure unexpected null previndex\n");
                }
            } else LogPrintf("rtb_ensure unexpected null miprev\n");
        }
     }*/
    return(pindex);
}
...
bool ProcessNewBlock(bool from_miner,int32_t height,CValidationState &state, CNode* pfrom, CBlock* pblock, bool fForceProcessing, CDiskBlockPos *dbp)
{
    ...
    hash = pblock->GetHash();
    {
        LOCK(cs_main);
        if ( chainActive.Tip() != 0 )
            rtb_currentheight_set(chainActive.Tip()->nHeight);
        checked = CheckBlock(&futureblock,height!=0?height:rtb_block2height(pblock),0,*pblock, state, verifier,0);
        bool fRequested = MarkBlockAsReceived(hash);
        fRequested |= fForceProcessing;
        if ( checked != 0 && rtb_checkPOW(0,0,pblock,height) < 0 ) //from_miner && ASSETCHAINS_STAKED == 0
        {
            checked = 0;
...
//void rtb_pindex_init(CBlockIndex *pindex,int32_t height);
...
bool static LoadBlockIndexDB()
{
    ...
    BOOST_FOREACH(const PAIRTYPE(uint256, CBlockIndex*)& item, mapBlockIndex)
    {
        ...
        //rtb_pindex_init(pindex,(int32_t)pindex->nHeight);
    }
    ...
    if ( chainName.isKMD() ) {
        ...
        int32_t longestchain = rtb_longestchain();
        // TODO: rtb_longestchain does not have the data it needs at the time LoadBlockIndexDB
...
bool LoadBlockIndex(bool reindex)
{
    ...
    RTB_LOADINGBLOCKS = true;
    if (!reindex && !LoadBlockIndexDB())
    {
        RTB_LOADINGBLOCKS = false;
        return false;
    }
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                if (mapBlockIndex.count(hash) == 0 || (mapBlockIndex[hash]->nStatus & BLOCK_HAVE_DATA) == 0) {
                    ...
                    if (state.IsError())
                        break;
                } else if (hash != chainparams.GetConsensus().hashGenesisBlock && rtb_blockheight(hash) % 1000 == 0) {
                    LogPrintf("Block Import: already had block %s at height %d\n", hash.ToString(), rtb_blockheight(hash));
                }
...
#include "rtb_nSPV_defs.h"
#include "rtb_nSPV.h"            // shared defines, structs, serdes, purge functions
#include "rtb_nSPV_fullnode.h"   // nSPV fullnode handling of the getnSPV request messages
#include "rtb_nSPV_superlite.h"  // nSPV superlite client, issuing requests and handling nSPV responses
#include "rtb_nSPV_wallet.h"     // nSPV_send and support functions, really all the rest is to support this
...
//LogPrintf("netmsg: %s\n", strCommand.c_str());
    ...
    {
        if ( RTB_NSPV == 0 )//&& RTB_INSYNC != 0 )
        {
            std::vector<uint8_t> payload;
            vRecv >> payload;
            rtb_nSPVreq(pfrom,payload);
        }
        return(true);
    }
    else if (GetBoolArg("-nspv_msg", DEFAULT_NSPV_PROCESSING) && strCommand == "nSPV")
    {
        if ( RTB_NSPV_SUPERLITE )
        {
            std::vector<uint8_t> payload;
            vRecv >> payload;
            rtb_nSPVresp(pfrom,payload);
        }
        return(true);
    }
    else if ( RTB_NSPV_SUPERLITE )
        return(true);
...
bool SendMessages(CNode* pto, bool fSendTrickle)
{
    const Consensus::Params& consensusParams = Params().GetConsensus();
    {
        ...
        /*CBlockIndex *pindex;
        if ( rtb_requestedhash != zero && rtb_requestedcount < 16 && (pindex= rtb_getblockindex(rtb_requestedhash)) != 0 )
        {
            LogPrint("net","rtb_requestedhash.%d request %s to nodeid.%d\n",rtb_requestedcount,rtb_requestedhash.ToString().c_str(),pto->GetId());
            LogPrintf("rtb_requestedhash.%d request %s to nodeid.%d\n",rtb_requestedcount,rtb_requestedhash.ToString().c_str(),pto->GetId());
            vGetData.push_back(CInv(MSG_BLOCK, rtb_requestedhash));
            MarkBlockAsInFlight(pto->GetId(), rtb_requestedhash, consensusParams, pindex);
            rtb_requestedcount++;
            if ( rtb_requestedcount > 16 )
            {
                memset(&rtb_requestedhash,0,sizeof(rtb_requestedhash));
                rtb_requestedcount = 0;
            }
