/******************************************************************************
 * Copyright © 2021 RTB Core Deveelopers                                   *
 *                                                                            *
 ...
 */
void rtb_state::AddCheckpoint(const notarized_checkpoint &in)
{
 ...
 */
int32_t rtb_state::NotarizedData(int32_t nHeight,uint256 *notarized_hashp,uint256 *notarized_desttxidp) const
{
    bool found = false;
 ...
 * @returns the notarized height
 */
int32_t rtb_state::NotarizedHeight(int32_t *prevMoMheightp,uint256 *hashp,uint256 *txidp)
{
    CBlockIndex *pindex;
    if ( (pindex= rtb_blockindex(last.notarized_hash)) == 0 || pindex->nHeight < 0 )
    {
        // found orphaned notarization, adjust the values in the rtb_state object
 ...
 */
int32_t rtb_state::PrevMoMHeight() const
{
    static uint256 zero;
    // shortcut
    if (last.MoM != zero)
 ...
 */
const notarized_checkpoint *rtb_state::CheckpointAtHeight(int32_t height) const
{
...
void rtb_state::clear_checkpoints() { NPOINTS.clear(); }
const uint256& rtb_state::LastNotarizedHash() const { return last.notarized_hash; }
void rtb_state::SetLastNotarizedHash(const uint256 &in) { last.notarized_hash = in; }
const uint256& rtb_state::LastNotarizedDestTxId() const { return last.notarized_desttxid; }
void rtb_state::SetLastNotarizedDestTxId(const uint256 &in) { last.notarized_desttxid = in; }
const uint256& rtb_state::LastNotarizedMoM() const { return last.MoM; }
void rtb_state::SetLastNotarizedMoM(const uint256 &in) { last.MoM = in; }
const int32_t& rtb_state::LastNotarizedHeight() const { return last.notarized_height; }
void rtb_state::SetLastNotarizedHeight(const int32_t in) { last.notarized_height = in; }
const int32_t& rtb_state::LastNotarizedMoMDepth() const { return last.MoMdepth; }
void rtb_state::SetLastNotarizedMoMDepth(const int32_t in) { last.MoMdepth =in; }
uint64_t rtb_state::NumCheckpoints() const { return NPOINTS.size(); }
