...
#include "rtb_utils.h" // dstr()
#include "rtb_hardfork.h"

#define RTB_INTEREST ((uint64_t)5000000) //((uint64_t)(0.05 * COIN))   // 5%

uint64_t _rtb_interestnew(int32_t txheight,uint64_t nValue,uint32_t nLockTime,uint32_t tiptime)
{
    int32_t minutes; 
    if ( nLockTime >= LOCKTIME_THRESHOLD 
            && tiptime > nLockTime 
            && (minutes= (tiptime - nLockTime) / 60) >= (RTB_MAXMEMPOOLTIME/60) )
    {
        if ( minutes > 365 * 24 * 60 )
            minutes = 365 * 24 * 60;
        if ( txheight >= 1000000 && minutes > 31 * 24 * 60 )
            minutes = 31 * 24 * 60;
        minutes -= ((RTB_MAXMEMPOOLTIME/60) - 1);
 ...
 */
uint64_t rtb_interestnew(int32_t txheight,uint64_t nValue,uint32_t nLockTime,uint32_t tiptime)
{
    if ( txheight < RTB_ENDOFERA 
            && nLockTime >= LOCKTIME_THRESHOLD 
            && tiptime != 0 
            && nLockTime < tiptime 
            && nValue >= 10*COIN )
        return _rtb_interestnew(txheight,nValue,nLockTime,tiptime);
    return 0;
}
 ...
 */
uint64_t rtb_interest(int32_t txheight,uint64_t nValue,uint32_t nLockTime,uint32_t tiptime)
{
    ...
    if ( nLockTime >= LOCKTIME_THRESHOLD && tiptime != 0 && nLockTime < tiptime && nValue >= 10*COIN )
    {
        ...
        if ( minutes >= 60 )
        {
            ...
            if ( denominator == 0 )
                denominator = 1; // max RTB_INTEREST per transfer, do it at least annually!
            if ( nValue > 25000LL*COIN )
            {
                ...
                if ( !exception )
                {
                    ...
                    else if ( txheight < 1000000 )
                    {
                        ...
                        uint64_t interestnew = _rtb_interestnew(txheight,nValue,nLockTime,tiptime);
                        ...
                        interest = _rtb_interestnew(txheight,nValue,nLockTime,tiptime);
                }
                else if ( txheight < 1000000 )
                {
                    uint64_t numerator = (nValue * RTB_INTEREST);
 ...
 * @returns locktime
 */
uint32_t rtb_interest_args(uint32_t *txheighttimep,int32_t *txheightp,uint32_t *tiptimep,uint64_t *valuep,
        uint256 hash,int32_t n)
{
    ...
    *valuep = 0;
    ...
    if ( n < tx.vout.size() )
    {
        CBlockIndex *pindex = rtb_getblockindex(hashBlock);
        if ( pindex != nullptr )
        {
 ...
 */
uint64_t rtb_accrued_interest(int32_t *txheightp,uint32_t *locktimep,uint256 hash,int32_t n,
        int32_t checkheight,uint64_t checkvalue,int32_t tipheight)
{
    ...
    uint64_t value;
    *locktimep = rtb_interest_args(&txheighttimep, txheightp, &tiptime, &value, hash, n);
    if ( *locktimep != 0 )
    {
        if ( (checkvalue == 0 || value == checkvalue) && (checkheight == 0 || *txheightp == checkheight) )
            return rtb_interest(*txheightp,value,*locktimep,tiptime);
        else 
            LogPrintf("rtb_accrued_interest value mismatch %llu vs %llu or height mismatch %d vs %d\n",(long long)value,(long long)checkvalue,*txheightp,checkheight);
    }
    return 0;
}
