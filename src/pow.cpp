...
unsigned int GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock, const Consensus::Params& params)
{
    ...
    if ( ASSETCHAINS_ADAPTIVEPOW > 0 )
    {
        ...
        {
            easy.SetCompact(RTB_MINDIFF_NBITS & (~3),&fNegative,&fOverflow);
            if ( pblock != 0 )
            {
                origtarget = bnTarget;
...
/* RTB_LOADINGBLOCKS moved from here to rtb_globals.cpp and made boolean */
...
bool CheckProofOfWork(const CBlockHeader &blkHeader, uint8_t *pubkey33, int32_t height, const Consensus::Params& params)
{
    ...
    if ( height > 34000 && chainName.isKMD() ) // 0 -> non-special notary
    {
        ...
        flag = rtb_eligiblenotary(pubkeys,mids,blocktimes,&nonzpkeys,height);
        special2 = rtb_is_special(pubkeys,mids,blocktimes,height,pubkey33,blkHeader.nTime);
        if ( notaryid >= 0 )
        {
            if ( height > 10000 && height < 80000 && (special != 0 || special2 > 0) )
                ...
                flag = (height > 1000000 || (height % RTB_ELECTION_GAP) > 64 || (height % RTB_ELECTION_GAP) == 0);
            ...
            if ( (flag != 0 || special2 > 0) && special2 != -2 )
            {
                ...
                bnTarget.SetCompact(RTB_MINDIFF_NBITS,&fNegative,&fOverflow);
            }
        }
    }
