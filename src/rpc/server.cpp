...
UniValue stop(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
   ...
   // Accept the deprecated and ignored 'detach' boolean argument
    ...
    StartShutdown();
    sprintf(buf,"%s server stopping", !chainName.isKMD() ? chainName.symbol().c_str() : "RTB");
    return buf;
}
...
{
    if ( chainName.isKMD() ) {
        return "> rtb-cli " + methodname + " " + args + "\n";
    } else {
        return "> rtb-cli -ac_name=" + strprintf("%s", chainName.symbol().c_str()) + " " + methodname + " " + args + "\n";
    }
}
...
{
    string daemon = "rtbd";
    string ticker = chainName.isKMD() ? "rtb" : chainName.symbol();
