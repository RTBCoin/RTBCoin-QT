...
int32_t rtb_longestchain()
{
    ...
    if ( depth == 0 )
    {
        ...
        BOOST_FOREACH(const CNodeStats& stats, vstats)
        {
            //fprintf(stderr,"rtb_longestchain iter.%d\n",n);
            CNodeStateStats statestats;
            bool fStateStats = GetNodeStateStats(stats.nodeid,statestats);
        ...
        if ( num > (n >> 1) )
        {
            if ( 0 && height != RTB_LONGESTCHAIN )
                LogPrintf("set %s RTB_LONGESTCHAIN <- %d\n",chainName.symbol().c_str(),height);
            RTB_LONGESTCHAIN = height;
            return(height);
        }
        RTB_LONGESTCHAIN = 0;
    }
    return(RTB_LONGESTCHAIN);
}
...
UniValue getaddednodeinfo(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "    \"addresses\" : [\n"
            "       {\n"
            "         \"address\" : \"192.168.0.201:8233\",  (string) The RTB server host and port\n"
