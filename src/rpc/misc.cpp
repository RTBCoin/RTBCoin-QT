...
extern bool IS_RTB_NOTARY;
extern int32_t RTB_LASTMINED,JUMBLR_PAUSE,RTB_LONGESTCHAIN,STAKED_NOTARY_ID,STAKED_ERA,RTB_INSYNC;
...
UniValue getiguanajson(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    ...
    // get the min sigs .. this always rounds UP so min sigs in iguana is +1 min sigs in rtbd, due to some rounding error.
    int minsigs;
...
//#endif
    ...
    obj.push_back(Pair("KMDversion", RTB_VERSION));
    obj.push_back(Pair("synced", RTB_INSYNC!=0));
    ...
    if ( RTB_NSPV_FULLNODE )
    {
        ...
        if (pwalletMain) {
            ...
            if ( chainName.isKMD() )
            {
                obj.push_back(Pair("interest",       ValueFromAmount(RTB_INTERESTSUM)));
                obj.push_back(Pair("balance",       ValueFromAmount(RTB_WALLETBALANCE))); //pwalletMain->GetBalance()
            }
            else
            {
        ...
        if ( (longestchain= RTB_LONGESTCHAIN) != 0 && chainActive.Height() > longestchain )
    ...
    obj.push_back(Pair("errors",        GetWarnings("statusbar")));
     if ( NOTARY_PUBKEY33[0] != 0 ) {
        ...
        } else if( (notaryid= rtb_whoami(pubkeystr,(int32_t)chainActive.Tip()->nHeight,rtb_chainactive_timestamp())) >= 0 )  {
            obj.push_back(Pair("notaryid",        notaryid));
            if ( RTB_LASTMINED != 0 )
                obj.push_back(Pair("lastmined", RTB_LASTMINED));
        }
...
UniValue coinsupply(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    ...
    if (height >= 0 && height <= currentHeight) {
        if ( (supply= rtb_coinsupply(&zfunds,&sproutfunds,height)) > 0 )
        {
            ...
            if ( ASSETCHAINS_BLOCKTIME > 0 )
            {
                ...
                {
                    supply1 = rtb_coinsupply(&zf1,&sf1,height - blocks_per_year/12);
                    supply3 = rtb_coinsupply(&zf3,&sf3,height - blocks_per_year/4);
                    supply12 = rtb_coinsupply(&zf12,&sf12,height - blocks_per_year);
...
UniValue validateaddress(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"rtbaddress\"\n"
            "\nReturn information about the given Komodo address.\n"
            "\nArguments:\n"
            "1. \"rtbaddress\"     (string, required) The Komodo address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"rtbaddress\",   (string) The Komodo address validated\n"
    ...
    if (isValid)
    {
        ...
        ret.push_back(Pair("segid", (int32_t)rtb_segid32((char *)params[0].get_str().c_str()) & 0x3f));
#ifdef ENABLE_WALLET
...
UniValue verifymessage(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"rtbaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"rtbaddress\"    (string, required) The Komodo address to use for the signature.\n"
...
UniValue getnotarypayinfo(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    ...
    notarycount = rtb_notaries(notarypubkeys, height, chainActive[height]->GetBlockTime());
    NotaryPay = rtb_notarypayamount(height, notarycount)*notarycount;
...
UniValue txnotarizedconfirmed(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    ...
    notarizedconfirmed=rtb_txnotarizedconfirmed(txid);
    UniValue result(UniValue::VOBJ);
