...
UniValue getgenerate(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "It is set with the command line argument -gen (or rtb.conf setting gen) and -mint\n"
    ...
    obj.push_back(Pair("numthreads",       (int64_t)RTB_MININGTHREADS));
    return obj;
}
...
std::shared_ptr<CBlock> generateBlock(CWallet* wallet, CValidationState* validationState)
{
    ...
    std::unique_ptr<CBlockTemplate> pblocktemplate(CreateNewBlockWithKey(reservekey,nHeight,RTB_MAXGPUCOUNT));
...
#else
        throw JSONRPCError(RPC_METHOD_NOT_FOUND, "rtbd compiled without wallet and -mineraddress not set");
#endif
    }
...
UniValue setgenerate(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    ...
    RTB_MININGTHREADS = (int32_t)nGenProcLimit;
...
UniValue genminingCSV(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    ...
    if ( (fp= fopen(fname,"wb")) != 0 )
    {
        ...
        height = rtb_nextheight();
        for (i=0; i<height; i++)
        {
            if ( (pindex= rtb_chainactive(i)) != 0 )
            {
...
//            "  \"coinbasevalue\" : n,               (numeric) maximum allowable input to coinbase transaction, including the generation award and transaction fees (in Satoshis)\n"
    ...
    {
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "RTB is not connected!");
    }
    ...
    if (chainName.isKMD() && IsInitialBlockDownload()) {
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "RTB is downloading blocks...");
    }
        ...
        (mempool.GetTransactionsUpdated() != nTransactionsUpdatedLast && GetTime() - nStart > 5))
    {
        ...
        CReserveKey reservekey(pwalletMain);
        LEAVE_CRITICAL_SECTION(cs_main);
        pblocktemplate = CreateNewBlockWithKey(reservekey,pindexPrevNew->nHeight+1,RTB_MAXGPUCOUNT,false);
#else
    ...
    if ( ASSETCHAINS_STAKED != 0 )
    {
        ...
        POWtarget = rtb_PoWtarget(&PoSperc,hashTarget,(int32_t)(pindexPrev->nHeight+1),ASSETCHAINS_STAKED,rtb_newStakerActive(chainActive.Height()+1, pblock->nTime));
    ...
    if ( strlen(ASSETCHAINS_OVERRIDE_PUBKEY.c_str()) == 66 || ASSETCHAINS_SCRIPTPUB.size() > 1 )
    {
        if ( ASSETCHAINS_FOUNDERS == 0 && ASSETCHAINS_COMMISSION != 0 )
        {
            ...
            if ( nHeight <= chainActive.Height() )
            {
                ...
                CBlock block;
                if ( rtb_blockload(block, pblockIndex) == 0 )
                    nFoundersReward = rtb_commission(&block, nHeight);
            }
        }
        else if ( ASSETCHAINS_FOUNDERS != 0 )
        {
            // Assetchains founders chains have a fixed reward so can be calculated at any given height.
            nFoundersReward = rtb_commission(0, nHeight);
        }
