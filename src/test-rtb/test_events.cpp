...
{
    rtb_state* state = rtb_stateptrget((char*)symbol);
 ...
 */
TEST(test_events, rtb_faststateinit_test)
{
    char symbol[] = "TST";
    chainName = assetchain("TST");
    RTB_EXTERNAL_NOTARIES = 1;
    IS_RTB_NOTARY = false;   // avoid calling rtb_verifynotarization
    ...
    try
    {
        ...
        // pubkey record
        {
            // create a binary file that should be readable by rtb
            ...
            clear_state(symbol); // clear rtb_state*
            // attempt to read the file
            rtb_state* state = rtb_stateptrget((char*)symbol);
            ASSERT_TRUE(state != nullptr);
            char* dest = nullptr;   // not used for 'P'
            // attempt to read the file
            int32_t result = rtb_faststateinit( state, full_filename.c_str(), symbol, dest);
            // compare results
            EXPECT_EQ(result, 1);
            /* old way
            // The first and second event should be pub keys
            EXPECT_EQ(state->Komodo_numevents, 1);
            rtb_event* ev1 = state->Komodo_events[0];
            ...
            */
            ...
            rtb::event_pubkeys& ev2 = 
                    static_cast<rtb::event_pubkeys&>( *state->events.front() );
            EXPECT_EQ(ev2.height, 10);
            EXPECT_EQ(ev2.type, rtb::rtb_event_type::EVENT_PUBKEYS);
        ...
        // notarized record
        {
            ...
            rtb_event* ev = state->Komodo_events[1];
            ...
            */
            ...
            rtb::event_notarized& ev2 = 
                    static_cast<rtb::event_notarized&>( *state->events.front() );
            EXPECT_EQ(ev2.height, 10);
            EXPECT_EQ(ev2.type, rtb::rtb_event_type::EVENT_NOTARIZED);
            // the serialized version should match the input
            EXPECT_TRUE(compare_serialization(full_filename, ev2));

            // check last nota read from rtb state:
        ...
        // notarized M record
        {
            ...
            rtb_event* ev = state->Komodo_events[2];
            EXPECT_EQ(ev->height, 1);
            ...
            */
            ...
            //std::advance(itr, 2);
            rtb::event_notarized& ev2 = static_cast<rtb::event_notarized&>( *state->events.front() );
            EXPECT_EQ(ev2.height, 10);
            EXPECT_EQ(ev2.type, rtb::rtb_event_type::EVENT_NOTARIZED);
            // the serialized version should match the input
            EXPECT_TRUE(compare_serialization(full_filename, ev2));
        ...
        // record type K (KMD height)
        {
            ...
            rtb_event* ev = state->Komodo_events[3];
            EXPECT_EQ(ev->height, 1);
            EXPECT_EQ(ev->type, 'K');            
            */
            ASSERT_TRUE(state->events.size() != 0); // prevent test crash
            ...
            //auto itr = state->events.begin();
            //std::advance(itr, 3);
            rtb::event_kmdheight& ev2 = static_cast<rtb::event_kmdheight&>( *state->events.front() );
            EXPECT_EQ(ev2.height, 10);
            EXPECT_EQ(ev2.type, rtb::rtb_event_type::EVENT_KMDHEIGHT);
            // the serialized version should match the input
            EXPECT_TRUE(compare_serialization(full_filename, ev2));
        }
        // record type T (KMD height with timestamp)
        {
            ...
            rtb_event* ev = state->Komodo_events[4];
            EXPECT_EQ(ev->height, 1);
        ...
        // record type R (opreturn)
        {
            ...
            rtb_event* ev = state->Komodo_events[5];
            EXPECT_EQ(ev->height, 1);
            EXPECT_EQ(ev->type, 'R');
            */
            ASSERT_TRUE(state->events.size() != 0); // prevent test crash
            ...
            auto itr = state->events.begin();
            std::advance(itr, 5);
            rtb::event_opreturn& ev2 = static_cast<rtb::event_opreturn&>( *state->events.front() );
            EXPECT_EQ(ev2.height, 10);
            EXPECT_EQ(ev2.type, rtb::rtb_event_type::EVENT_OPRETURN);
            // the serialized version should match the input
            EXPECT_TRUE(compare_serialization(full_filename, ev2));
        }
        // record type V
        {
            ...
            rtb_event* ev = state->Komodo_events[6];
            EXPECT_EQ(ev->height, 1);
            EXPECT_EQ(ev->type, 'V');
            */
            ASSERT_TRUE(state->events.size() != 0); // prevent test crash
            ...
            //auto itr = state->events.begin();
            // std::advance(itr, 6);
            rtb::event_pricefeed& ev2 = static_cast<rtb::event_pricefeed&>( *state->events.front() );
            EXPECT_EQ(ev2.height, 10);
            EXPECT_EQ(ev2.type, rtb::rtb_event_type::EVENT_PRICEFEED);
            // the serialized version should match the input
            EXPECT_TRUE(compare_serialization(full_filename, ev2));
        }
        ...
        // all together in 1 file
        {
            ...
            rtb_event* ev1 = state->Komodo_events[7];
            EXPECT_EQ(ev1->height, 1);
            EXPECT_EQ(ev1->type, 'P');
            */
            ASSERT_TRUE(state->events.size() != 0); // prevent test crash
            ...
            std::advance(itr, 0);
            {
                ...
                EXPECT_EQ( (**itr).type, rtb::rtb_event_type::EVENT_PUBKEYS);
                itr++;
            }
            {
                EXPECT_EQ( (**itr).height, 10);
                EXPECT_EQ( (**itr).type, rtb::rtb_event_type::EVENT_NOTARIZED);
                itr++;
            }
            {
                EXPECT_EQ( (**itr).height, 10);
            ...
            {
                EXPECT_EQ( (**itr).height, 10);
                EXPECT_EQ( (**itr).type, rtb::rtb_event_type::EVENT_KMDHEIGHT);
                itr++;
            }
            {
                EXPECT_EQ( (**itr).height, 10);
            ...
            {
                EXPECT_EQ( (**itr).height, 10);
                EXPECT_EQ( (**itr).type, rtb::rtb_event_type::EVENT_OPRETURN);
                itr++;
            }
            {
                EXPECT_EQ( (**itr).height, 10);
                EXPECT_EQ( (**itr).type, rtb::rtb_event_type::EVENT_PRICEFEED);
                itr++;
            }
        }
    } 
...
// same test for KMD
TEST(test_events, rtb_faststateinit_test_kmd)
{
    // Nothing should be added to events if this is the rtb chain

    char symbol[] = "KMD";
    chainName = assetchain();
    RTB_EXTERNAL_NOTARIES = 0;
    IS_RTB_NOTARY = false;   // avoid calling rtb_verifynotarization
    ...
    {
        {
            ...
            // check that the new way is the same
            EXPECT_EQ(state->events.size(), 1);
            rtb::event_pubkeys& ev = static_cast<rtb::event_pubkeys&>( *state->events.front() );
            EXPECT_EQ(ev.height, 10);
            EXPECT_EQ(ev.type, rtb::rtb_event_type::EVENT_PUBKEYS);
            std::fclose(fp);
            // verify files still exists
            EXPECT_TRUE(boost::filesystem::exists(full_filename));
        }
...
{
    // make an object
    rtb::event_pubkeys pk1(1);
    pk1.num = 2;
    memset(pk1.pubkeys[0], 1, 33);
    memset(pk1.pubkeys[1], 2, 33);
    // make a copy
    std::vector<std::shared_ptr<rtb::event>> events;
    events.push_back( std::make_shared<rtb::event_pubkeys>(pk1) );
    rtb::event_pubkeys& pk2 = static_cast<rtb::event_pubkeys&>(*events.front());
    // are they equal?
