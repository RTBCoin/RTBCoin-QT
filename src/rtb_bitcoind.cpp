...
#include "rtb.h" // rtb_voutupdate()
#include "rtb_utils.h" // OS_milliseconds
...
int32_t notarizedtxid_height(char *dest,char *txidstr,int32_t *kmdnotarized_heightp)
{
    ...
    *kmdnotarized_heightp = 0;
    ...
    if ( userpass[0] != 0 )
    {
        ...
        {
            if ( (jsonstr= rtb_issuemethod(userpass,(char *)"getinfo",params,port)) != 0 )
            {
                //LogPrintf("(%s)\n",jsonstr);
            ...
            if ( (jsonstr= rtb_issuemethod(userpass,(char *)"getblockchaininfo",params,port)) != 0 )
            {
                //LogPrintf("(%s)\n",jsonstr);
                if ( (json= cJSON_Parse(jsonstr)) != 0 )
                {
        ...
        if ( (jsonstr= rtb_issuemethod(userpass,(char *)"getrawtransaction",params,port)) != 0 )
        {
            //LogPrintf("(%s)\n",jsonstr);
            if ( (json= cJSON_Parse(jsonstr)) != 0 )
            {
...
void rtb_reconsiderblock(uint256 blockhash)
{
    ...
    if ( (jsonstr= rtb_issuemethod(ASSETCHAINS_USERPASS,(char *)"reconsiderblock",params,ASSETCHAINS_RPCPORT)) != 0 )
    {
        //LogPrintf("rtb_reconsiderblock.(%s) (%s %u) -> (%s)\n",params,ASSETCHAINS_USERPASS,ASSETCHAINS_RPCPORT,jsonstr);
        free(jsonstr);
    }
    //LogPrintf("rtb_reconsiderblock.(%s) (%s %u) -> NULL\n",params,ASSETCHAINS_USERPASS,ASSETCHAINS_RPCPORT);
}
...
int32_t rtb_verifynotarization(const char *symbol,const char *dest,int32_t height,int32_t NOTARIZED_HEIGHT,uint256 NOTARIZED_HASH,uint256 NOTARIZED_DESTTXID)
{
    ...
    if ( strcmp(dest,"KMD") == 0 )
    {
        if ( KMDUSERPASS[0] != 0 )
        {
            if ( !chainName.isKMD() )
            {
                jsonstr = rtb_issuemethod(KMDUSERPASS,(char *)"getrawtransaction",params,KMD_PORT);
    ...
    else if ( strcmp(dest,"BTC") == 0 )     // Note: this should work for LTC too (BTC is used as an alias for LTC)
    {
        if ( BTCUSERPASS[0] != 0 )
        {
            jsonstr = rtb_issuemethod(BTCUSERPASS,(char *)"getrawtransaction",params,DEST_PORT);
        }
        //else jsonstr = _dex_getrawtransaction();
        else return(0);
    }
            ...
            if ( (txjson= jobj(json,(char *)"result")) != 0 && (vouts= jarray(&n,txjson,(char *)"vout")) != nullptr )
            {
                ...
                if ( (skey= jobj(vout,(char *)"scriptPubKey")) != 0 )
                {
                    if ( (hexstr= jstr(skey,(char *)"hex")) != 0 )
                    {
                        ...
                        retval = rtb_verifynotarizedscript(height,script,len,NOTARIZED_HASH);
                    }
                }
            }
            free_json(txjson);
...
uint64_t rtb_seed(int32_t height)
{
    ...
    /*if ( 0 ) // problem during init time, seeds are needed for loading blockindex, so null seeds...
     ...
     hash = _rtb_getblockhash(height);
     if ( memcmp(&hash,&zero,sizeof(hash)) == 0 )
     hash = rtb_getblockhash(height);
     int32_t i;
     for (i=0; i<32; i++)
     LogPrintf("%02x",((uint8_t *)&hash)[i]);
     LogPrintf(" seed.%d\n",height);
...
uint32_t rtb_txtime2(uint64_t *valuep,uint256 hash,int32_t n,char *destaddr)
{
    ...
    *valuep = 0;
    ...
    if ( (pindex= rtb_getblockindex(hashBlock)) != 0 )
        txtime = pindex->nTime;
...
{
    if ( timestamp > nStakedDecemberHardforkTimestamp || rtb_heightstamp(height) > nStakedDecemberHardforkTimestamp ) //December 2019 hardfork
        return(1);
    else return(0);
}
...
{    
    return rtb_newStakerActive(height, timestamp) == 1;
}

bool rtb_checkopret(CBlock *pblock, CScript &merkleroot)
{
    merkleroot = pblock->vtx.back().vout.back().scriptPubKey;
    return(merkleroot.IsOpReturn() && merkleroot == rtb_makeopret(pblock, false));
}
...
int32_t rtb_isPoS(CBlock *pblock, int32_t height,CTxDestination *addressout)
{
    ...
    if ( ASSETCHAINS_STAKED != 0 )
    {
        ...
        if ( n > 1 && pblock->vtx[n-1].vin.size() == 1 && pblock->vtx[n-1].vout.size() == 1+rtb_hasOpRet(height,pblock->nTime) )
        {
            txid = pblock->vtx[n-1].vin[0].prevout.hash;
            vout = pblock->vtx[n-1].vin[0].prevout.n;
            txtime = rtb_txtime(opret,&value,txid,vout,destaddr);
            if ( ExtractDestination(pblock->vtx[n-1].vout[0].scriptPubKey,voutaddress) )
            {
                ...
                if ( rtb_newStakerActive(height, pblock->nTime) != 0 ) 
                {
                    if ( DecodeStakingOpRet(pblock->vtx[n-1].vout[1].scriptPubKey, merkleroot) != 0 && rtb_calcmerkleroot(pblock, pblock->hashPrevBlock, height, false, pblock->vtx[0].vout[0].scriptPubKey) == merkleroot )
                    {
                        return(1);
                    }
                }
...
{
    CBlockIndex *ptr;
    if ( height > 0 && (ptr= rtb_chainactive(height)) != nullptr )
        return(ptr->nTime);
    return 0;
}
...
void rtb_index2pubkey33(uint8_t *pubkey33,CBlockIndex *pindex,int32_t height)
{
    ...
    if ( pindex != 0 )
    {
        if ( rtb_blockload(block,pindex) == 0 )
            rtb_block2pubkey33(pubkey33,&block);
    }
}
...
int32_t rtb_eligiblenotary(uint8_t pubkeys[66][33],int32_t *mids,uint32_t blocktimes[66],int32_t *nonzpkeysp,int32_t height)
{
    ...
    n = rtb_notaries(notarypubs33,height,0);
    for (i=duplicate=0; i<66; i++)
    {
        if ( (pindex= rtb_chainactive(height-i)) != 0 )
        {
            blocktimes[i] = pindex->nTime;
            if ( rtb_blockload(block,pindex) == 0 )
            {
                rtb_block2pubkey33(pubkeys[i],&block);
                for (j=0; j<n; j++)
                {
...
int32_t rtb_minerids(uint8_t *minerids,int32_t height,int32_t width)
{
    ...
    numnotaries = rtb_notaries(notarypubs33,height,0);
    for (i=nonz=0; i<width; i++)
    {
        if ( height-i <= 0 )
            continue;
        if ( (pindex= rtb_chainactive(height-width+i+1)) != 0 )
        {
            if ( rtb_blockload(block,pindex) == 0 )
            {
                rtb_block2pubkey33(pubkey33,&block);
...
int32_t rtb_is_special(uint8_t pubkeys[66][33],int32_t mids[66],uint32_t blocktimes[66],int32_t height,uint8_t pubkey33[33],uint32_t blocktime)
{
    ...
    rtb_chosennotary(&notaryid,height,pubkey33,blocktimes[0]);
    if ( height >= 82000 )
    {
        if ( notaryid >= 0 )
        {
    ...
    else
    {
        if ( height >= 34000 && notaryid >= 0 )
        {
            ...
            for (i=1; i<limit; i++)
            {
                rtb_chosennotary(&nid,height-i,pubkey33,blocktimes[i]);
                if ( nid == notaryid )
                {
                    //for (j=0; j<66; j++)
                    //    LogPrintf("%d ",mids[j]);
...
int32_t rtb_MoM(int32_t *notarized_heightp,uint256 *MoMp,uint256 *kmdtxidp,int32_t nHeight,uint256 *MoMoMp,int32_t *MoMoMoffsetp,int32_t *MoMoMdepthp,int32_t *kmdstartip,int32_t *kmdendip)
{
    ...
    depth = rtb_MoMdata(&notarized_ht,&MoM,&kmdtxid,nHeight,MoMoMp,MoMoMoffsetp,MoMoMdepthp,kmdstartip,kmdendip);
...
bool rtb_checkpoint(int32_t *notarized_heightp, int32_t nHeight, uint256 hash)
{
    ...
    int32_t notarized_height = rtb_notarizeddata(pindex->nHeight,&notarized_hash,&notarized_desttxid);
    *notarized_heightp = notarized_height;
    ...
    {
        //verify that the block info returned from rtb_notarizeddata matches the actual block
...
int32_t rtb_nextheight()
{
    ...
    if ( (pindex= chainActive.Tip()) != 0 && (ht= pindex->nHeight) > 0 )
        return(ht+1);
    else return(rtb_longestchain() + 1);
}
 ...
 * @return 1 if this chain's height >= rtb_longestchain(), otherwise 0
 */
int32_t rtb_isrealtime(int32_t *kmdheightp)
{
    AssertLockHeld(cs_main);

    rtb_state *sp = rtb_stateptrget( (char*)"KMD");
    if ( sp != nullptr )
        *kmdheightp = sp->CURRENT_HEIGHT;
    else 
        *kmdheightp = 0;
    CBlockIndex *pindex = chainActive.Tip();
    if ( pindex != nullptr && pindex->nHeight >= (int32_t)rtb_longestchain() )
        return 1;
    return 0;
}
...
{
    if ( RTB_REWIND == 0 && chainName.isKMD() && (int64_t)tx.nLockTime >= LOCKTIME_THRESHOLD ) //1473793441 )
    {
        if ( txheight > 246748 ) // a long time ago
        {
            if ( txheight < 247205 ) // a long time ago
                cmptime -= 16000; // subtract about 4 1/2 hours
            if ( (int64_t)tx.nLockTime < cmptime-RTB_MAXMEMPOOLTIME )
            {  
                // transaction has been in mempool for more than an hour
                if ( tx.nLockTime != 1477258935 )
                {
                    LogPrintf("rtb_validate_interest.%d reject.%d [%d] locktime %u cmp2.%u\n",1,txheight,(int32_t)(tx.nLockTime - (cmptime-RTB_MAXMEMPOOLTIME)),(uint32_t)tx.nLockTime,cmptime);
                }
                return false;
            }
        }
...
int8_t rtb_segid(int32_t nocache,int32_t height)
{
    ...
    if ( height > 0 && (pindex= rtb_chainactive(height)) != 0 )
    {
        if ( nocache == 0 && pindex->segid >= -1 )
            return(pindex->segid);
        if ( rtb_blockload(block,pindex) == 0 )
        {
            newStakerActive = rtb_newStakerActive(height, block.nTime);
            txn_count = block.vtx.size();
            if ( txn_count > 1 && block.vtx[txn_count-1].vin.size() == 1 && block.vtx[txn_count-1].vout.size() == 1+rtb_hasOpRet(height,pindex->nTime) )
            {
                ...
                if ( ExtractDestination(block.vtx[txn_count-1].vout[0].scriptPubKey,voutaddress) )
                {
                    ...
                    {
                        segid = rtb_segid32(voutaddr) & 0x3f;
                        //LogPrintf( "rtb_segid: ht.%i --> %i\n",height,pindex->segid);
                    }
                } //else LogPrintf("rtb_segid ht.%d couldnt extract voutaddress\n",height);
            }
        }
        // The new staker sets segid in rtb_checkPOW, this persists after restart by being saved in the blockindex for blocks past the HF timestamp, to keep backwards compatibility.
...
void rtb_segids(uint8_t *hashbuf,int32_t height,int32_t n)
{
    ...
    else
    {
        ...
        for (i=0; i<n; i++)
        {
            hashbuf[i] = (uint8_t)rtb_segid(0,height+i);
...
arith_uint256 rtb_PoWtarget(int32_t *percPoSp,arith_uint256 target,int32_t height,int32_t goalperc,int32_t newStakerActive)
{
    ...
    *percPoSp = percPoS = 0;
    ...
    for (i=n=m=0; i<100; i++)
    {
        ...
        if ( ht <= 1 )
            continue;
        if ( (pindex= rtb_chainactive(ht)) != 0 )
        {
            if ( rtb_segid(0,ht) >= 0 )
            {
                n++;
...
uint32_t rtb_stake(int32_t validateflag,arith_uint256 bnTarget,int32_t nHeight,uint256 txid,int32_t vout,uint32_t blocktime,uint32_t prevtime,char *destaddr,int32_t PoSperc)
{
    ...
    txtime = rtb_txtime2(&value,txid,vout,address);
    if ( validateflag == 0 )
    {
    ...
    {
        //LogPrintf("rtb_stake null %.8f %u %u %u\n",dstr(value),txtime,blocktime,prevtime);
        return(0);
    }
    if ( value < SATOSHIDEN )
        return(0);
    ...
    if ( (minage= nHeight*3) > 6000 ) // about 100 blocks
        minage = 6000;
    rtb_segids(hashbuf,nHeight-101,100);
    segid32 = rtb_stakehash(&hash,address,hashbuf,txid,vout);
...
int32_t rtb_is_PoSblock(int32_t slowflag,int32_t height,CBlock *pblock,arith_uint256 bnTarget,arith_uint256 bhash)
{
    ...
    int32_t newStakerActive = rtb_newStakerActive(height, pblock->nTime);
    // Get PoSperc and POW Target. slowflag only here, calling it when blocks out of order causes problems.
    if ( slowflag != 0 )
    {
        POWTarget = rtb_PoWtarget(&PoSperc,bnTarget,height,ASSETCHAINS_STAKED,newStakerActive);
    }
    else 
    {
        // checks opret merkle root and existence of staking tx.
        return(rtb_isPoS(pblock,height,0));
    }
    txn_count = pblock->vtx.size();
    ...
    if ( txn_count > 1 && pblock->vtx[txn_count-1].vin.size() == 1 && pblock->vtx[txn_count-1].vout.size() == 1+rtb_hasOpRet(height,pblock->nTime) )
    {
        ...
        {
            if ( rtb_isPoS(pblock,height,0) != 0 ) 
            {
                // checks utxo is eligible to stake this block
                eligible = rtb_stake(1,bnTarget,height,txid,vout,pblock->nTime,prevtime+ASSETCHAINS_STAKED_BLOCK_FUTURE_HALF,(char *)"",PoSperc); 
            }
            if ( eligible == 0 || eligible > pblock->nTime )
            {
                if ( 0 && ASSETCHAINS_STAKED < 100 ) 
                    LogPrintf("rtb_is_PoSblock PoS failure ht.%d eligible.%u vs blocktime.%u, lag.%d -> check to see if it is PoW block\n",height,eligible,(uint32_t)pblock->nTime,(int32_t)(eligible - pblock->nTime));
            }
            else 
            {
                isPoS = 1;
...
uint64_t rtb_notarypayamount(int32_t nHeight, int64_t notarycount)
{
    ...
    if ( notarycount == 0 )
    {
        LogPrintf( "rtb_notarypayamount failed num notaries is 0!\n");
        return(0);
    }
...
int32_t rtb_getnotarizedheight(uint32_t timestamp,int32_t height, uint8_t *script, int32_t len)
{
    ...
    if ( len >= sizeof(uint32_t) && len <= sizeof(scriptbuf) )
    {
        ...
        if ( rtb_voutupdate(true,&isratification,0,scriptbuf,len,height,uint256(),1,1,&voutmask,&specialtx,&notarizedheight,0,1,0,timestamp) != -2 )
        {
...
uint64_t rtb_notarypay(CMutableTransaction &txNew, std::vector<int8_t> &NotarisationNotaries, uint32_t timestamp, int32_t height, uint8_t *script, int32_t len)
{
    ...
    numSN = rtb_notaries(notarypubkeys, height, timestamp);
    ...
    int32_t notarizedheight = rtb_getnotarizedheight(timestamp, height, script, len);
    ...
    AmountToPay = rtb_notarypayamount(height,NotarisationNotaries.size());
    if ( AmountToPay == 0 )
        return(0);
...
uint64_t rtb_checknotarypay(CBlock *pblock,int32_t height)
{
    ...
    if ( pblock->vtx[1].vout.size() == 2 && pblock->vtx[1].vout[1].nValue == 0 )
    {
        ...
        if ( script[0] == OP_RETURN )
        {
            ...
            totalsats = rtb_notarypay(txNew, NotarisationNotaries, pblock->nTime, height, script, scriptlen);
        } 
        else 
        {
...
bool rtb_appendACscriptpub()
{
    ...
    if ( ASSETCHAINS_SCRIPTPUB[ASSETCHAINS_SCRIPTPUB.back()] == 49 && ASSETCHAINS_SCRIPTPUB[ASSETCHAINS_SCRIPTPUB.back()-1] == 51 )
    {
        ...
        if ( myGetTransaction(RTB_EARLYTXID,tx,blockhash) && mapBlockIndex[blockhash]->nHeight < RTB_EARLYTXID_HEIGHT )
        {
        ...
        LogPrintf( "could not get RTB_EARLYTXID.%s OP_RETURN data. Restart with correct txid!\n", RTB_EARLYTXID.GetHex().c_str());
        StartShutdown();
    }
    return false;
}
...
{
    AssertLockHeld(cs_main);
    if ( RTB_EARLYTXID == zeroid )
    {
        LogPrintf( "Restart deamon with -earlytxid.\n");
        StartShutdown();
        return;
    }
    if ( chainActive.Height() < RTB_EARLYTXID_HEIGHT )
    {
    ...
    if ( myGetTransaction(RTB_EARLYTXID,tx,blockhash) && mapBlockIndex[blockhash]->nHeight < RTB_EARLYTXID_HEIGHT )
    {
        ...
        if ( i < tx.vout.size() )
        {
            RTB_EARLYTXID_SCRIPTPUB = CScript(tx.vout[i].scriptPubKey.begin()+3, tx.vout[i].scriptPubKey.end());
            LogPrintf( "RTB_EARLYTXID_SCRIPTPUB.%s\n", HexStr(RTB_EARLYTXID_SCRIPTPUB.begin(),RTB_EARLYTXID_SCRIPTPUB.end()).c_str());
            return;
        }
    }
...
int64_t rtb_checkcommission(CBlock *pblock,int32_t height)
{
    ...
    {
        checktoshis = rtb_commission(pblock,height);
        if ( checktoshis >= 10000 && pblock->vtx[0].vout.size() < 2 )
        {
            //LogPrintf("rtb_checkcommission vsize.%d height.%d commission %.8f\n",(int32_t)pblock->vtx[0].vout.size(),height,(double)checktoshis/COIN);
            return(-1);
        }
        else if ( checktoshis != 0 )
        {
            ...
            {
                static bool didinit = false;
                if ( !didinit && height > RTB_EARLYTXID_HEIGHT && RTB_EARLYTXID != zeroid && rtb_appendACscriptpub() )
                {
...
bool RTB_TEST_ASSETCHAIN_SKIP_POW = 0;
...
int32_t rtb_checkPOW(int64_t stakeTxValue, int32_t slowflag,CBlock *pblock,int32_t height)
{
    ...
    if ( RTB_TEST_ASSETCHAIN_SKIP_POW == 0 && Params().NetworkIDString() == "regtest" )
        RTB_TEST_ASSETCHAIN_SKIP_POW = 1;
    if ( !CheckEquihashSolution(pblock, Params()) )
    {
        LogPrintf("rtb_checkPOW slowflag.%d ht.%d CheckEquihashSolution failed\n",slowflag,height);
        return(-1);
    }
    hash = pblock->GetHash();
    bnTarget.SetCompact(pblock->nBits,&fNegative,&fOverflow);
    bhash = UintToArith256(hash);
    possible = rtb_block2pubkey33(pubkey33,pblock);
    if ( height == 0 )
    {
        if ( slowflag != 0 )
        {
    ...
    if ( ( !chainName.isKMD() || height > 792000) && bhash > bnTarget )
    {
        ...
        {
            if ( (n= rtb_notaries(pubkeys,height,pblock->nTime)) > 0 )
            {
                for (i=0; i<n; i++)
        ...
        {
            if ( RTB_TEST_ASSETCHAIN_SKIP_POW )
                return(0);
            if ( ASSETCHAINS_STAKED == 0 ) // rtb_is_PoSblock will check bnTarget for staked chains
                return(-1);
        }
    }
    ...
    if ( ASSETCHAINS_STAKED != 0 && height >= 2 ) // must PoS or have at least 16x better PoW
    {
        ...
        pindex = it != mapBlockIndex.end() ? it->second : NULL;
        int32_t newStakerActive = rtb_newStakerActive(height, pblock->nTime);
        if ( (is_PoSblock= rtb_is_PoSblock(slowflag,height,pblock,bnTarget,bhash)) == 0 )
        {
            ...
            {
                bnTarget = rtb_PoWtarget(&PoSperc,bnTarget,height,ASSETCHAINS_STAKED,newStakerActive);
                if ( bhash > bnTarget ) 
                {
                    for (i=31; i>=16; i--)
    ...
    if ( failed == 0 && ASSETCHAINS_COMMISSION != 0 ) 
    {
        ...
        {
            if ( rtb_checkcommission(pblock,height) < 0 )
                return(-1);
        }
    }
    ...
    if ( failed == 0 && ASSETCHAINS_NOTARY_PAY[0] != 0 && pblock->vtx.size() > 1 )
    {
        ...
        if ( slowflag == 0 && pblock->vtx[0].vout.size() > 1 )
        {
            ...
            if ( !rtb_is_notarytx(pblock->vtx[1]) )
            {
            ...
            // Check min sigs.
            int8_t numSN = 0; uint8_t notarypubkeys[64][33] = {0};
            numSN = rtb_notaries(notarypubkeys, height, pblock->nTime);
...
//LogPrintf("rtb_checkPOW possible.%d slowflag.%d ht.%d notaryid.%d failed.%d\n",possible,slowflag,height,notaryid,failed);
...
int32_t rtb_acpublic(uint32_t tiptime)
{
    ...
    if ( acpublic == 0 )
    {
        ...
        if ( (chainName.isKMD() || chainName.isSymbol("ZEX")) && tiptime >= RTB_SAPLING_DEADLINE )
            acpublic = 1;
    }
    return(acpublic);
}
...
int64_t rtb_coinsupply(int64_t *zfundsp,int64_t *sproutfundsp,int32_t height)
{
    ...
    *zfundsp = *sproutfundsp = 0;
    if ( (pindex= rtb_chainactive(height)) != 0 )
    {
        while ( pindex != 0 && pindex->nHeight > 0 )
        {
            ...
            {
                if ( rtb_blockload(block,pindex) == 0 )
                    pindex->newcoins = rtb_newcoins(&pindex->zfunds,&pindex->sproutfunds,pindex->nHeight,&block);
                else
                {
...
{
    uint256 hash; uint32_t segid32; rtb_staking kp;
    segid32 = rtb_stakehash(&hash,address,hashbuf,txid,vout);
...
int32_t rtb_staked(CMutableTransaction &txNew,uint32_t nBits,uint32_t *blocktimep,uint32_t *txtimep,uint256 *utxotxidp,int32_t *utxovoutp,uint64_t *utxovaluep,uint8_t *utxosig, uint256 merkleroot)
{
    ...
    thread_local std::vector<rtb_staking> array; 
    ...
    *utxovaluep = 0;
    ...
    //tmpTarget = rtb_PoWtarget(&PoSperc,bnTarget,nHeight,ASSETCHAINS_STAKED);
    ...
    if ( array.size() != 0 )
    {
        ...
        if ( ReadBlockFromDisk(block, pblockindex, 1) && rtb_isPoS(&block, nHeight, &addressout) != 0 && IsMine(*pwalletMain,addressout) != 0 )
        {
              resetstaker = true;
    ...
    if ( resetstaker || array.size() == 0 || time(NULL) > lasttime+600 )
    {
        ...
        BOOST_FOREACH(const COutput& out, vecOutputs)
        {
            ...
            {
                //LogPrintf("rtb_staked invalid depth %d\n",(int32_t)out.nDepth);
                continue;
            }
            ...
            if ( ExtractDestination(pk,address) != 0 )
            {
                if ( IsMine(*pwalletMain,address) == 0 )
                    continue;
                if ( myGetTransaction(out.tx->GetHash(),tx,hashBlock) != 0 && (pindex= rtb_getblockindex(hashBlock)) != 0 )
                {
                    rtb_addutxo(array,(uint32_t)pindex->nTime,(uint64_t)nValue,out.tx->GetHash(),out.i,(char *)CBitcoinAddress(address).ToString().c_str(),hashbuf,(CScript)pk);
    ...
    for (i=winners=0; i<array.size(); i++)
    {
        ...
        rtb_staking &kp = array[i];
        eligible = rtb_stake(0,bnTarget,nHeight,kp.txid,kp.vout,0,(uint32_t)tipindex->nTime+ASSETCHAINS_STAKED_BLOCK_FUTURE_HALF,kp.address,PoSperc);
        if ( eligible > 0 )
        {
            besttime = 0;
            if ( eligible == rtb_stake(1,bnTarget,nHeight,kp.txid,kp.vout,eligible,(uint32_t)tipindex->nTime+ASSETCHAINS_STAKED_BLOCK_FUTURE_HALF,kp.address,PoSperc) )
            {
    ...
    if ( earliest != 0 )
    {
        ...
        if ( (newStakerActive= rtb_newStakerActive(nHeight,earliest)) != 0 )
        {
