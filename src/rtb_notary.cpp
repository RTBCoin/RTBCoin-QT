...
#include "rtb.h" // rtb_stateupdate()
#include "rtb_structs.h" // RTB_NOTARIES_HARDCODED
#include "rtb_utils.h" // rtb_stateptr
#include "rtb_bitcoind.h"

//struct knotaries_entry *Pubkeys;  // todo remove

// statics used within this .cpp for caching purposes
static int didinit; // see rtb_init
static uint8_t kmd_pubkeys[NUM_KMD_SEASONS][64][33]; // see rtb_notaries
static int32_t hwmheight; // highest height ever passed to rtb_notariesinit
static int32_t hadnotarization; // used in rtb_dpowconfs
static bool didinit_NOTARIES[NUM_KMD_SEASONS]; // rtb_notaries()
...
{
    int32_t htind = height / RTB_ELECTION_GAP;
    if ( htind >= RTB_MAXBLOCKS / RTB_ELECTION_GAP )
        htind = (RTB_MAXBLOCKS / RTB_ELECTION_GAP) - 1;
    return htind;
}
...
int32_t rtb_isnotaryvout(char *coinaddr,uint32_t tiptime) // from ac_private chains only
{
    int32_t season = getacseason(tiptime);
    if ( NOTARY_ADDRESSES[season-1][0][0] == 0 )
    {
        uint8_t pubkeys[64][33];
        rtb_notaries(pubkeys,0,tiptime);
    }
...
int32_t rtb_notaries(uint8_t pubkeys[64][33],int32_t height,uint32_t timestamp)
{
    ...
    if ( chainName.isKMD() )   
        ...
        timestamp = rtb_heightstamp(height); // derive the timestamp from the passed-in height

    // If this chain is not a staked chain, use the normal RTB logic to determine notaries. 
    ...
    if ( is_STAKED(chainName.symbol()) == 0 )
    {
        ...
        if ( chainName.isKMD() )
        {
            ...
            if ( height >= RTB_NOTARIES_HARDCODED )
    ...
    std::lock_guard<std::mutex> lock(rtb_mutex);
...
int32_t rtb_electednotary(int32_t *numnotariesp,uint8_t *pubkey33,int32_t height,uint32_t timestamp)
{
    ...
    n = rtb_notaries(pubkeys,height,timestamp);
    *numnotariesp = n;
    for (i=0; i<n; i++)
    {
...
{
    if ( Pubkeys == 0 )
        Pubkeys = (knotaries_entry *)calloc(1 + (RTB_MAXBLOCKS / RTB_ELECTION_GAP),sizeof(*Pubkeys));
    knotaries_entry N;
    memset(&N,0,sizeof(N));
    ...
    if ( origheight > 0 )
    {
        int32_t height = (origheight + RTB_ELECTION_GAP/2);
        height /= RTB_ELECTION_GAP;
        height = ((height + 1) * RTB_ELECTION_GAP);
    ...
    }
    {
        ...
        N.numnotaries = num;
        for (int32_t i=htind; i<RTB_MAXBLOCKS / RTB_ELECTION_GAP; i++)
        {
            ...
            Pubkeys[i] = N;
            Pubkeys[i].height = i * RTB_ELECTION_GAP;
        }
    }
...
int32_t rtb_chosennotary(int32_t *notaryidp,int32_t height,uint8_t *pubkey33,uint32_t timestamp)
{
    ...
    *notaryidp = -1;
    if ( height < 0 )//|| height >= RTB_MAXBLOCKS )
    {
        LogPrintf("rtb_chosennotary ht.%d illegal\n",height);
        return(-1);
    }
    if ( height >= RTB_NOTARIES_HARDCODED || !chainName.isKMD() )
    {
        if ( (*notaryidp= rtb_electednotary(&numnotaries,pubkey33,height,timestamp)) >= 0 && numnotaries != 0 )
        {
 ...
 */
const notarized_checkpoint *rtb_npptr(int32_t height)
{
    char symbol[RTB_ASSETCHAIN_MAXLEN];
    char dest[RTB_ASSETCHAIN_MAXLEN]; 

    rtb_state *sp = rtb_stateptr(symbol, dest);
    if ( sp != nullptr )
    {
...
int32_t rtb_dpowconfs(int32_t txheight,int32_t numconfs)
{
    ...
    if ( RTB_DPOWCONFS != 0 && txheight > 0 && numconfs > 0 && (sp= rtb_stateptr(symbol,dest)) != nullptr )
    {
...
{
    const notarized_checkpoint *np = rtb_npptr(height);
    if ( np != nullptr )
    {
        ...
        uint256 notarized_hash,uint256 notarized_desttxid,uint256 MoM,int32_t MoMdepth)
{
    ...
    {
        LogPrintf("rtb_notarized_update REJECT notarized_height %d > %d nHeight\n",notarized_height,nHeight);
        return;
    }
...
void rtb_init(int32_t height)
{
    ...
    if ( !didinit )
    {
        ...
        if ( height >= 0 )
        {
            ...
            rtb_notarysinit(0,pubkeys,count);
        }
        didinit = true;
        rtb_stateupdate(0,0,0,0,zero,0,0,0,0,0,0,0,0,zero,0);
    }
}

/****
 * A nasty hack to reset statics in this file.
 * Do not put this in rtb_notary.h, as it should only be used for testing
 */
void rtb_notaries_uninit()
{
    didinit = 0;
    hwmheight = 0;
    hadnotarization = 0;
