...
#include "rtb_utils.h" // portable_mutex_lock
#include "rtb_curve25519.h" // for rtb_kvsigverify
#include <mutex>
 ...
 */
uint256 rtb_kvsig(uint8_t *buf,int32_t len,uint256 _privkey)
{
 ...
 */
int32_t rtb_kvnumdays(uint32_t flags)
{
    int32_t numdays;
...
{
    return(rtb_kvnumdays(flags) * RTB_KVDURATION);
}
 ...
 */
int32_t rtb_kvsearch(uint256 *pubkeyp, int32_t current_height, uint32_t *flagsp,
        int32_t *heightp, uint8_t value[IGUANA_MAXSCRIPTSIZE], uint8_t *key, int32_t keylen)
{
    ...
    *flagsp = 0;
    ...
    HASH_FIND(hh,RTB_KV,key,keylen,ptr);
    if ( ptr != nullptr )
    {
        int32_t duration = rtb_kvduration(ptr->flags);
        if ( current_height > (ptr->height + duration) )
        {
            // entry has expired, remove it
            HASH_DELETE(hh,RTB_KV,ptr);
            if ( ptr->value != 0 )
                free(ptr->value);
            if ( ptr->key != 0 )
                free(ptr->key);
 ...
 */
void rtb_kvupdate(uint8_t *opretbuf,int32_t opretlen,uint64_t value)
{
    static uint256 zeroes;
    ...
    if ( keylen+13 > opretlen )
    {
        ...
        if ( ++counter < 1 )
            LogPrintf("rtb_kvupdate: keylen.%d + 13 > opretlen.%d, this can be ignored\n",keylen,opretlen);
        return;
    }
    uint8_t *valueptr = &key[keylen];
    uint64_t fee = rtb_kvfee(flags,opretlen,keylen);
    if ( value >= fee )
    {
                ...
                || opretlen == coresize+2*sizeof(uint256) )
        {
            ...
            uint256 refpubkey;
            int32_t kvheight;
            int32_t refvaluesize = rtb_kvsearch(&refpubkey,height,
            ...
            else if ( ptr == 0 )
            {
                ...
                ptr = (rtb_kv *)calloc(1,sizeof(*ptr));
                ptr->key = (uint8_t *)calloc(1,keylen);
                ptr->keylen = keylen;
                memcpy(ptr->key,key,keylen);
                newflag = true;
                HASH_ADD_KEYPTR(hh,RTB_KV,ptr->key,ptr->keylen,ptr);
            }
            if ( newflag || (ptr->flags & RTB_KVPROTECTED) == 0 ) // can we edit the value?
            {
