...
#include "rtb_notary.h" // rtb_npptr

struct rtb_ccdata *CC_data;
// int32_t CC_firstheight;
pthread_mutex_t RTB_CC_mutex; 
...
{
    struct rtb_ccdata_entry *allMoMs=0; struct rtb_ccdata *ccdata,*tmpptr; int32_t i,num,max;
    bool fMutated; std::vector<uint256> tree, leaves;
    num = max = 0;
    portable_mutex_lock(&RTB_CC_mutex);
    DL_FOREACH_SAFE(CC_data,ccdata,tmpptr)
    {
        if ( ccdata->MoMdata.height <= kmdendi && ccdata->MoMdata.height >= kmdstarti )
        {
            ...
            {
                max += 100;
                allMoMs = (struct rtb_ccdata_entry *)realloc(allMoMs,max * sizeof(*allMoMs));
            }
    ...
    portable_mutex_unlock(&RTB_CC_mutex);
    if ( (*nump= num) > 0 )
    {
        for (i=0; i<num; i++)
...
int32_t rtb_addpair(struct rtb_ccdataMoMoM *mdata,int32_t notarized_height,int32_t offset,int32_t maxpairs)
{
    if ( maxpairs >= 0) {
        ...
        {
            maxpairs += 100;
            mdata->pairs = (struct rtb_ccdatapair *)realloc(mdata->pairs,sizeof(*mdata->pairs)*maxpairs);
            //LogPrintf("pairs reallocated to %p num.%d\n",mdata->pairs,mdata->numpairs);
        }
    } else {
        LogPrintf("rtb_addpair.maxpairs %d must be >= 0\n",(int32_t)maxpairs);
        return(-1);
    }
...
{
    uint8_t hexdata[8192]; struct rtb_ccdata *ccdata,*tmpptr; int32_t len,maxpairs,i,retval=-1,depth,starti,endi,CCid=0; struct rtb_ccdata_entry *allMoMs;
    ...
    {
        if ( (allMoMs= rtb_allMoMs(&depth,&mdata->MoMoM,starti,endi)) != 0 )
        {
            mdata->MoMoMdepth = depth;
            for (i=0; i<depth; i++)
            {
                if ( strcmp(symbol,allMoMs[i].symbol) == 0 )
                    maxpairs = rtb_addpair(mdata,allMoMs[i].notarized_height,i,maxpairs);
            }
            if ( mdata->numpairs > 0 )
            {
...
{
    struct rtb_ccdata *ccdata,*tmpptr;
    if ( chainName.isKMD() )
    {
        portable_mutex_lock(&RTB_CC_mutex);
        DL_FOREACH_SAFE(CC_data,ccdata,tmpptr)
