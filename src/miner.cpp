...
int32_t rtb_waituntilelegible(uint32_t blocktime, int32_t stakeHeight, uint32_t delay)
{
...
CBlockTemplate* CreateNewBlock(const CPubKey _pk,const CScript& _scriptPubKeyIn, int32_t gpucount, bool isStake)
{
    ...
    CBlockIndex* pindexPrev = 0;
    {
        ...
        if ( ASSETCHAINS_NOTARY_PAY[0] != 0 )
        {
            ...
            numSN = rtb_notaries(notarypubkeys, nHeight, pblock->nTime);
        }
             ...
             mi != mempool.mapTx.end(); ++mi)
        {
            ...
            if (chainName.isKMD() && !rtb_validate_interest(tx, nHeight, cmptime))
            {
                LogPrintf("%s: rtb_validate_interest failure txid.%s nHeight.%d nTime.%u vs locktime.%u (cmptime.%lu)\n", __func__, tx.GetHash().ToString(), nHeight, (uint32_t)pblock->nTime, (uint32_t)tx.nLockTime, cmptime);
                continue;
            }
            ...
            if (tx.IsCoinImport())
            {
                ...
                if ( numSN != 0 && notarypubkeys[0][0] != 0 && rtb_is_notarytx(tx) == 1 )
            ...
            if ( fNotarisation ) 
            {
                ...
                if ( tx.vout.size() == 2 && tx.vout[1].nValue == 0 )
                {
                    ...
                    if ( script[0] == OP_RETURN )
                    {
                        ...
                        int32_t notarizedheight = rtb_getnotarizedheight(pblock->nTime, nHeight, script, scriptlen);
        ...
        if ( !chainName.isKMD() && isStake )
        {
            ...
            blocktime = GetTime();
            uint256 merkleroot = rtb_calcmerkleroot(pblock, pindexPrev->GetBlockHash(), nHeight, true, scriptPubKeyIn);
            ...
            */
            ...
            siglen = rtb_staked(txStaked, pblock->nBits, &blocktime, &txtime, &utxotxid, &utxovout, &utxovalue, utxosig, merkleroot);
            if ( rtb_newStakerActive(nHeight, blocktime) != 0 )
                nFees += utxovalue;
            ...
            if ( rtb_waituntilelegible(blocktime, stakeHeight, delay) == 0 )
                return(0);
        ...
        if ( chainName.isKMD() && IS_RTB_NOTARY && My_notaryid >= 0 )
            txNew.vout[0].nValue += 5000;
        pblock->vtx[0] = txNew;

        uint64_t commission;
        if ( nHeight > 1 && !chainName.isKMD() && (ASSETCHAINS_OVERRIDE_PUBKEY33[0] != 0 || ASSETCHAINS_SCRIPTPUB.size() > 1) && (ASSETCHAINS_COMMISSION != 0 || ASSETCHAINS_FOUNDERS_REWARD != 0)  && (commission= rtb_commission((CBlock*)&pblocktemplate->block,(int32_t)nHeight)) != 0 )
        {
            uint8_t *ptr;
            txNew.vout.resize(2);
            ...
            if ( ASSETCHAINS_SCRIPTPUB.size() > 1 )
            {
                ...
                if ( !didinit && nHeight > RTB_EARLYTXID_HEIGHT && RTB_EARLYTXID != zeroid && rtb_appendACscriptpub() )
                {
        ...
        else if ( (uint64_t)(txNew.vout[0].nValue) >= ASSETCHAINS_TIMELOCKGTE)
        {
            ...
            opretScript.AddCheckLockTimeVerify(rtb_block_unlocktime(nHeight));
        ...
        else if ( fNotarisationBlock && ASSETCHAINS_NOTARY_PAY[0] != 0 && pblock->vtx[1].vout.size() == 2 && pblock->vtx[1].vout[1].nValue == 0 )
        {
            ...
            if ( script[0] == OP_RETURN )
            {
                uint64_t totalsats = rtb_notarypay(txNew, NotarisationNotaries, pblock->nTime, nHeight, script, scriptlen);
                if ( totalsats == 0 )
                {
        ...
        if ( chainName.isKMD() || ASSETCHAINS_STAKED == 0 || RTB_MININGTHREADS > 0 )
        {
        ...
        if ( chainName.isKMD() && IS_RTB_NOTARY && My_notaryid >= 0 )
        {
            ...
            if ( Mining_height > nDecemberHardforkHeight ) //December 2019 hardfork
                opret = rtb_makeopret(pblock, true);
            else
                opret.clear();

            if (rtb_notaryvin(txNotary, NOTARY_PUBKEY33, opret, pblock->nTime) > 0)
            {
                CAmount txfees = 5000;
        ...
        else if ( ASSETCHAINS_CC == 0 && pindexPrev != 0 && ASSETCHAINS_STAKED == 0 && (!chainName.isKMD() || !IS_RTB_NOTARY || My_notaryid < 0) )
        {
            CValidationState state;
...
void rtb_broadcast(CBlock *pblock,int32_t limit)
{
...
#endif // ENABLE_WALLET
{
    ...
    // Found a solution
    {
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
        {
            ...
            return error("RTBMiner: generated block is stale");
        }
    }
    ...
    if (!ProcessNewBlock(1,chainActive.Tip()->nHeight+1,state, NULL, pblock, true, NULL))
        return error("RTBMiner: ProcessNewBlock, block not accepted");

    TrackMinedBlock(pblock->GetHash());
    //rtb_broadcast(pblock,16);
    return true;
}

int32_t FOUND_BLOCK,RTB_MAYBEMINED;
extern int32_t RTB_LASTMINED,RTB_INSYNC;
...
#endif
{
    LogPrintf("RTBMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("rtb-miner");
    const CChainParams& chainparams = Params();
    ...
    uint8_t *script; uint64_t total; int32_t i,j,gpucount=RTB_MAXGPUCOUNT,notaryid = -1;
    while ( (ASSETCHAIN_INIT == 0 || RTB_INITDONE == 0) )
    {
        //sleep(1);
        boost::this_thread::sleep_for(boost::chrono::seconds(1)); // allow to interrupt

        if ( rtb_baseid(chainName.symbol().c_str()) < 0 )
            break;
    }
    if ( chainName.isKMD() )
    {
        ...
        rtb_chosennotary(&notaryid,newHeight,NOTARY_PUBKEY33,timePast);
    }
...
//                                                                       [&m_cs, &cancelSolver](const uint256& hashNewTip) mutable {
    ...
    try {
        ...
        while (true)
        {
            ...
            CBlockTemplate *ptr = CreateNewBlockWithKey(reservekey, pindexPrev->nHeight+1, gpucount, ASSETCHAINS_STAKED != 0 && RTB_MININGTHREADS == 0);
#else
            ...
            {
                if (GetArg("-mineraddress", "").empty()) {
                    LogPrintf("Error in RTBMiner: Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                } else {
                    // Should never reach here, because -mineraddress validity is checked in init.cpp
                    LogPrintf("Error in RTBMiner: Invalid -mineraddress\n");
                }
                return;
            }
            ...
            if ( (chainName.isKMD() && notaryid >= 0 && Mining_height > nDecemberHardforkHeight ) || (ASSETCHAINS_STAKED != 0 && rtb_newStakerActive(Mining_height, pblock->nTime) != 0) ) //December 2019 hardfork
                nExtraNonce = 0;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);
            //LogPrintf("Running RTBMiner.%s with %u transactions in block\n",solver.c_str(),(int32_t)pblock->vtx.size());
            LogPrintf("Running RTBMiner.%s with %u transactions in block (%u bytes)\n",solver.c_str(),pblock->vtx.size(),::GetSerializeSize(*pblock,SER_NETWORK,PROTOCOL_VERSION));
            //
            // Search
            //
            ...
            {
                j = 65;
                if ( (Mining_height >= 235300 && Mining_height < 236000) || (Mining_height % RTB_ELECTION_GAP) > 64 || (Mining_height % RTB_ELECTION_GAP) == 0 || Mining_height > 1000000 )
                {
                    ...
                    if ( notaryid <= 3 || notaryid == 32 || (notaryid >= 43 && notaryid <= 45) || notaryid == 51 || notaryid == 52 || notaryid == 56 || notaryid == 57 )
                        dispflag = 1;
                    rtb_eligiblenotary(pubkeys,mids,blocktimes,&nonzpkeys,Mining_height);
                    if ( nonzpkeys > 0 )
                    {
                        for (i=0; i<33; i++)
                            if( pubkeys[0][i] != 0 )
                    ...
                    if ((Mining_height >= 235300 && Mining_height < 236000) ||
                        (j == 65 && Mining_height > RTB_MAYBEMINED + 1 && Mining_height > RTB_LASTMINED + 64))
                    {
                        HASHTarget = arith_uint256().SetCompact(RTB_MINDIFF_NBITS);
            ...
            if ( ASSETCHAINS_STAKED > 0 )
            {
                ...
                HASHTarget_POW = rtb_PoWtarget(&percPoS,HASHTarget,Mining_height,ASSETCHAINS_STAKED,rtb_newStakerActive(Mining_height, pblock->nTime));
                HASHTarget.SetCompact(RTB_MINDIFF_NBITS,&fNegative,&fOverflow);
            ...
            while (true)
            {
                ...
                if ( RTB_MININGTHREADS > 0 && ASSETCHAINS_STAKED > 0 && ASSETCHAINS_STAKED < 100 && Mining_height > 10 )
                ...
                (std::vector<unsigned char> soln) {
                    ...
                    if ( IS_RTB_NOTARY && B.nTime > GetTime() )
                    {
                    ...
                    {
                        if ( RTB_MININGTHREADS == 0 ) // we are staking 
                        {
                            // Need to rebuild block if the found solution for PoS, meets POW target, otherwise it will be rejected. 
                            if ( ASSETCHAINS_STAKED < 100 && rtb_newStakerActive(Mining_height,pblock->nTime) != 0 && h < hashTarget_POW )
                            {
                            ...
                            if ( rtb_waituntilelegible(B.nTime, Mining_height, ASSETCHAINS_STAKED_BLOCK_FUTURE_MAX) == 0 )
                                return(false);
                        }
        ...
        catch (const std::runtime_error &e)
        {
            miningTimer.stop();
            c.disconnect();
            LogPrintf("RTBMiner runtime error: %s\n", e.what());
            return;
        }
        miningTimer.stop();
        c.disconnect();
