...
/****
 * @brief get the OS-specific default rtbd data directory
 * @note Windows: be "C:\Users\[username]\AppData\Roaming\RTB"
 * @note Mac: ~/Library/Application Support/RTB
 * @note Unix: ~/.rtb
 * @returns the default path to the RTB data directory
 */
...
{    
    ...
    if (chainName.isKMD())
        ...
        return pathRet / ".rtb" / chainName.symbol();
#endif
}
 ...
 * @returns the full OS-specific data directory including RTB (i.e. "~/.rtb")
 */
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    {
        // Don't overwrite existing settings so command line settings override rtb.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "rtbd.pid"));
...
std::string LicenseInfo()
{
    ...
    const std::string URL_SOURCE_CODE = "<https://github.com/KomodoPlatform/RTBCoin>";
    ...
    return "\n" +
           ...
           FormatParagraph(strprintf(_("Copyright (C) 2022-%i RTB Coin developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Check for untranslated substitution to make sure RTB Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("RTB Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The RTB Core developers";
    }
