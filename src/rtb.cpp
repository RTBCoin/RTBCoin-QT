...
//int32_t RTB_EXTERNAL_NOTARIES = 0; //todo remove
...
{
    char symbol[RTB_ASSETCHAIN_MAXLEN],dest[RTB_ASSETCHAIN_MAXLEN]; struct rtb_state *sp;
    if ( (sp= rtb_stateptr(symbol,dest)) != 0 )
    ...
    try
    {
        if ( (func= fgetc(fp)) != EOF )
        {
            ...
            if ( fread(&ht,1,sizeof(ht),fp) != sizeof(ht) )
                throw rtb::parse_error("Unable to read height from file");
            if ( func == 'P' )
            {
                rtb::event_pubkeys pk(fp, ht);
                if ( (RTB_EXTERNAL_NOTARIES && matched ) || (strcmp(symbol,"KMD") == 0 && !RTB_EXTERNAL_NOTARIES) )
                {
                    rtb_eventadd_pubkeys(sp, symbol, ht, pk);
                }
            }
            else if ( func == 'N' || func == 'M' )
            {
                rtb::event_notarized evt(fp, ht, dest, func == 'M');
                rtb_eventadd_notarized(sp, symbol, ht, evt);
            }
            else if ( func == 'U' ) // deprecated
            {
                rtb::event_u evt(fp, ht);
            }
            else if ( func == 'K' || func == 'T')
            {
                rtb::event_kmdheight evt(fp, ht, func == 'T');
                rtb_eventadd_kmdheight(sp, symbol, ht, evt);
            }
            else if ( func == 'R' )
            {
                rtb::event_opreturn evt(fp, ht);
                // check for oversized opret
                if ( evt.opret.size() < 16384*4 )
                    rtb_eventadd_opreturn(sp, symbol, ht, evt);
            }
            else if ( func == 'D' )
            {
            ...
            {
                rtb::event_pricefeed evt(fp, ht);
                rtb_eventadd_pricefeed(sp, symbol, ht, evt);
            }
            else if ( func == 'B' ) {
                // can be written but not processed on read
            }
            else {
                throw rtb::parse_error("Unable to parse state file: unknown event");
            }
        } // retrieved the func
    }
    catch(const rtb::parse_error& pe)
    {
        LogPrintf("Error occurred in parsestatefile: %s\n", pe.what());
        LogPrintf("%s file is invalid. Komodod will be stopped. Please remove %s and %s.ind files and start the daemon\n", RTB_STATE_FILENAME, RTB_STATE_FILENAME, RTB_STATE_FILENAME);
        uiInterface.ThreadSafeMessageBox(strprintf("Please remove %s and %s.ind files and restart", RTB_STATE_FILENAME, RTB_STATE_FILENAME), "", CClientUIInterface::MSG_ERROR);
        StartShutdown();            
        func = -1;
    }
    return func;
    ...
    try
    {
        ...
        if ( fpos < datalen )
        {
            ...
            if ( mem_read(ht, filedata, fpos, datalen) != sizeof(ht) )
                throw rtb::parse_error("Unable to parse height from file data");
            if ( func == 'P' )
            {
                rtb::event_pubkeys pk(filedata, fpos, datalen, ht);
                if ( (RTB_EXTERNAL_NOTARIES && matched ) || (strcmp(symbol,"KMD") == 0 && !RTB_EXTERNAL_NOTARIES) )
                {
                    rtb_eventadd_pubkeys(sp, symbol, ht, pk);
                }
            ...
            {
                rtb::event_notarized ntz(filedata, fpos, datalen, ht, dest, func == 'M');
                rtb_eventadd_notarized(sp, symbol, ht, ntz);
            }
            else if ( func == 'U' ) // deprecated
            {
                rtb::event_u u(filedata, fpos, datalen, ht);
            }
            else if ( func == 'K' || func == 'T' )
            {
                rtb::event_kmdheight kmd_ht(filedata, fpos, datalen, ht, func == 'T');
                rtb_eventadd_kmdheight(sp, symbol, ht, kmd_ht);
            }
            else if ( func == 'R' )
            {
                rtb::event_opreturn opret(filedata, fpos, datalen, ht);
                rtb_eventadd_opreturn(sp, symbol, ht, opret);
            }
            else if ( func == 'D' )
            {
            ...
            {
                rtb::event_pricefeed pf(filedata, fpos, datalen, ht);
                rtb_eventadd_pricefeed(sp, symbol, ht, pf);
            }
            else if ( func == 'B' ) {
                // can be written but not processed on read
            }
            else {
                throw rtb::parse_error("Unable to parse file data: unknown event");
            }
            *fposp = fpos;
        }
    }
...
void rtb_stateupdate(int32_t height,uint8_t notarypubs[][33],uint8_t numnotaries,
        ...
        uint8_t *opretbuf,uint16_t opretlen,uint16_t vout,uint256 MoM,int32_t MoMdepth)
{
    ...
    static uint256 zero;
    struct rtb_state *sp; 
    char fname[MAX_STATEFNAME+1],symbol[RTB_ASSETCHAIN_MAXLEN],dest[RTB_ASSETCHAIN_MAXLEN]; 
    int32_t ht,func;
    uint8_t num,pubkeys[64][33];

    if ( didinit == 0 )
    {
        portable_mutex_init(&RTB_CC_mutex);
        didinit = 1;
    }
    if ( (sp= rtb_stateptr(symbol,dest)) == 0 )
    {
        RTB_INITDONE = (uint32_t)time(NULL);
        LogPrintf("[%s] no rtb_stateptr\n",chainName.symbol().c_str());
        return;
    }
    if ( fp == 0 )
    {
        rtb_statefname(fname, chainName.symbol().c_str(), RTB_STATE_FILENAME);
        if ( (fp= fopen(fname,"rb+")) != nullptr )
        {
            if ( rtb_faststateinit(sp, fname, symbol, dest) )
                fseek(fp,0,SEEK_END);
            else
            {
                // unable to use faststateinit, so try again only slower
                LogPrintf("rtb_faststateinit retval.-1\n");
                while (!ShutdownRequested() && rtb_parsestatefile(sp,fp,symbol,dest) >= 0)
                    ;
            }
            LogPrintf("rtb read last notarised height %d from %s\n", sp->LastNotarizedHeight(), RTB_STATE_FILENAME);
        } 
        else 
    ...
    if ( fp != 0 ) // write out funcid, height, other fields, call side effect function
    {
        if ( KMDheight != 0 )
        {
            rtb::event_kmdheight kmd_ht(height);
            kmd_ht.kheight = KMDheight;
            kmd_ht.timestamp = KMDtimestamp;
            write_event(kmd_ht, fp);
            rtb_eventadd_kmdheight(sp,symbol,height,kmd_ht);
        }
        else if ( opretbuf != 0 && opretlen > 0 )
        {
            rtb::event_opreturn evt(height);
            evt.txid = txhash;
            evt.vout = vout;
        ...
        else if ( pvals != 0 && numpvals > 0 )
        {
            ...
            if ( nonz >= 32 )
            {
                rtb::event_pricefeed evt(height);
                evt.num = numpvals;
                for( uint8_t i = 0; i < evt.num; ++i)
                    evt.prices[i] = pvals[i];
                write_event(evt, fp);
                rtb_eventadd_pricefeed(sp,symbol,height,evt);
            }
        }
        else if ( height != 0 )
        {
            if ( sp != nullptr )
            {
                rtb::event_notarized evt(height, dest);
                ...
                write_event(evt, fp);
                rtb_eventadd_notarized(sp,symbol,height,evt);
            }
        }
        fflush(fp);
    }
...
int32_t rtb_validate_chain(uint256 srchash,int32_t notarized_height)
{
    static int32_t last_rewind; int32_t rewindtarget; CBlockIndex *pindex; struct rtb_state *sp; char symbol[RTB_ASSETCHAIN_MAXLEN],dest[RTB_ASSETCHAIN_MAXLEN];
    if ( (sp= rtb_stateptr(symbol,dest)) == 0 )
        return(0);
    if ( IsInitialBlockDownload() == 0 && ((pindex= rtb_getblockindex(srchash)) == 0 || pindex->nHeight != notarized_height) )
    {
        ...
        if ( rewindtarget != 0 && rewindtarget > RTB_REWIND && rewindtarget > last_rewind )
        {
            if ( last_rewind != 0 )
            {
...
namespace {
    ...
    std::string GetChainNameFromScript(uint8_t* scriptbuf, int32_t scriptlen, size_t offsetInScript)
    {
        ...
        const size_t maxChainName = sizeof(rtb_ccdata::symbol);
...
int32_t rtb_voutupdate(bool fJustCheck,int32_t *isratificationp,int32_t notaryid,uint8_t *scriptbuf,
        ...
        uint64_t signedmask,uint32_t timestamp)
{
    ...
    int32_t opretlen,nid,offset,k,MoMdepth,matched,len = 0; uint256 MoM,srchash,desttxid; uint8_t crypto777[33]; struct rtb_state *sp; char symbol[RTB_ASSETCHAIN_MAXLEN],dest[RTB_ASSETCHAIN_MAXLEN];
    if ( (sp= rtb_stateptr(symbol,dest)) == 0 )
        return(-1);
    if ( scriptlen == 35 && scriptbuf[0] == 33 && scriptbuf[34] == 0xac )
    {
        if ( i == 0 && j == 0 && memcmp(NOTARY_PUBKEY33,scriptbuf+1,33) == 0 && IS_RTB_NOTARY )
        {
            LogPrintf("%s RTB_LASTMINED.%d -> %d\n",chainName.symbol().c_str(),RTB_LASTMINED,height);
            prevRTB_LASTMINED = RTB_LASTMINED;
            RTB_LASTMINED = height;
        }
        ...
        else if ( rtb_chosennotary(&nid,height,scriptbuf + 1,timestamp) >= 0 )
        {
    ...
    {
        struct rtb_ccdata ccdata; struct rtb_ccdataMoMoM MoMoMdata;
        ...
        if ( j == 1 && opretlen >= len+offset-opoffset )
        {
            ...
            if ( matched == 0 && signedmask != 0 && bitweight(signedmask) >= RTB_MINRATIFY )
                notarized = 1;
            ...
            if ( matched != 0 )
                validated = rtb_validate_chain(srchash,*notarizedheightp);
            else validated = 1;
            ...
            if ( notarized != 0 && validated != 0 )
            {
                    ...
                    if ( len+sizeof(ccdata.CCid)-opoffset <= opretlen )
                    {
                        ...
                        if ( !chainName.isKMD() )
                        {
                            ...
                            if ( len+48-opoffset <= opretlen && chainName.isSymbol(ccdata.symbol) )
                            {
                                ...
                                {
                                    MoMoMdata.pairs = (struct rtb_ccdatapair *)calloc(MoMoMdata.numpairs,sizeof(*MoMoMdata.pairs));
                ...
                if ( matched != 0 && *notarizedheightp > sp->LastNotarizedHeight() && *notarizedheightp < height )
                {
                    ...
                    rtb_stateupdate(height,0,0,0,zero,0,0,0,0,0,0,0,0,sp->LastNotarizedMoM(),sp->LastNotarizedMoMDepth());
                    ...
                    if ( chainName.isKMD() )
                    {
                        if ( signedfp == 0 )
                        {
                            ...
                            rtb_statefname(fname,chainName.symbol().c_str(),(char *)"signedmasks");
...
// with more than RTB_MINRATIFY pay2pubkey outputs -> ratify
...
int32_t rtb_notarycmp(uint8_t *scriptPubKey,int32_t scriptlen,uint8_t pubkeys[64][33],int32_t numnotaries,uint8_t rmd160[20])
{
    int32_t i;
...
*/
/* 
    JMJ: Moved hwmheight out of rtb_connectblock to allow testing. 
    Adjusting this should only be done by rtb_connectblock or a unit test
*/
...
int32_t rtb_connectblock(bool fJustCheck, CBlockIndex *pindex,CBlock& block)
{
    ...
    uint64_t signedmask,voutmask; char symbol[RTB_ASSETCHAIN_MAXLEN],dest[RTB_ASSETCHAIN_MAXLEN]; struct rtb_state *sp;
    ...
    if ( pindex == 0 )
    {
        LogPrintf("rtb_connectblock null pindex\n");
        return(0);
    }
    memset(&zero,0,sizeof(zero));
    rtb_init(pindex->nHeight);
    RTB_INITDONE = (uint32_t)time(NULL);
    if ( (sp= rtb_stateptr(symbol,dest)) == 0 )
    {
        LogPrintf("unexpected null rtbstateptr.[%s]\n",chainName.symbol().c_str());
        return(0);
    }
    ...
    numnotaries = rtb_notaries(pubkeys,pindex->nHeight,pindex->GetBlockTime());
    ...
    else
    {
        if ( pindex->nHeight != hwmheight )
        {
            ...
            rtb_purge_ccdata((int32_t)pindex->nHeight);
            hwmheight = pindex->nHeight;
        }
        if (!fJustCheck)
        {
            rtb_event_rewind(sp,symbol,pindex->nHeight);
            rtb_stateupdate(pindex->nHeight,0,0,0,zero,0,0,-pindex->nHeight,pindex->nTime,0,0,0,0,zero,0);
        }
    }
    ...
    if ( pindex != 0 )
    {
        ...
        for (i=0; i<txn_count; i++)
        {
            ...
            for (j=0; j<numvins; j++)
            {
                ...
                {
                    if ( (k= rtb_notarycmp(scriptPubKey,scriptlen,pubkeys,numnotaries,rmd160)) >= 0 )
                        signedmask |= (1LL << k);
            ...
            if ( ((height < 90000 || (signedmask & 1) != 0) && numvalid >= RTB_MINRATIFY) 
                    || (numvalid >= RTB_MINRATIFY && !chainName.isKMD()) 
            ...
            for (j=0; j<numvouts; j++)
            {
                ...
                if ( len >= sizeof(uint32_t) && len <= sizeof(scriptbuf) )
                {
                    ...
                    notaryid = rtb_voutupdate(fJustCheck,&isratification,notaryid,scriptbuf,len,height,txhash,i,j,&voutmask,&specialtx,&notarizedheight,(uint64_t)block.vtx[i].vout[j].nValue,notarized,signedmask,(uint32_t)chainActive.Tip()->GetBlockTime());
            ...
            if ( !fJustCheck && (notarized != 0 && (notarizedheight != 0 || specialtx != 0)) )
            {
                ...
                if ( specialtx != 0 && isratification != 0 && numvouts > 2 )
                {
                    if (chainName.isKMD() && height < 100000)
                    {
                        ...
                        if ( ((signedmask & 1) != 0 && numvalid >= RTB_MINRATIFY) || bitweight(signedmask) > (numnotaries/3) )
                        {
                            memset(&txhash,0,sizeof(txhash));
                            rtb_stateupdate(height,pubkeys,numvalid,0,txhash,0,0,0,0,0,0,0,0,zero,0);
                            LogPrintf("RATIFIED! >>>>>>>>>> new notaries.%d newheight.%d from height.%d\n",numvalid,(((height+RTB_ELECTION_GAP/2)/RTB_ELECTION_GAP)+1)*RTB_ELECTION_GAP,height);
                        } else LogPrintf("signedmask.%llx numvalid.%d wt.%d numnotaries.%d\n",(long long)signedmask,numvalid,bitweight(signedmask),numnotaries);
                    }
                }
            }
        }
        if ( !fJustCheck && IS_RTB_NOTARY && chainName.isKMD() )
            LogPrintf("%s ht.%d\n",chainName.ToString().c_str(),height);
        if ( !fJustCheck && pindex->nHeight == hwmheight )
            rtb_stateupdate(height,0,0,0,zero,0,0,height,(uint32_t)pindex->nTime,0,0,0,0,zero,0);
    } 
    else 
        { LogPrintf("rtb_connectblock: unexpected null pindex\n"); return(0); }
    if (fJustCheck)
    {
        ...
        LogPrintf("rtb_connectblock: unxexpected behaviour when fJustCheck == true, report blackjok3rtt plz ! \n");
