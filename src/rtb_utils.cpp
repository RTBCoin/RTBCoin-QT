...
int32_t rtb_opreturnscript(uint8_t *script,uint8_t type,uint8_t *opret,int32_t opretlen)
{
    int32_t offset = 0;
    script[offset++] = 0x6a;
    opretlen++;
...
uint64_t rtb_block_prg(uint32_t nHeight)
{
...
int64_t rtb_block_unlocktime(uint32_t nHeight)
{
    ...
    {
        unlocktime = rtb_block_prg(nHeight) % (ASSETCHAINS_TIMEUNLOCKTO - ASSETCHAINS_TIMEUNLOCKFROM);
 ...
 * @return the RPC port
 */
uint16_t _rtb_userpass(char *username,char *password,FILE *fp)
{
    uint16_t port = 0;
...
void rtb_statefname(char *fname, const char *symbol, const char *str)
{
    int32_t n,len;
    ...
    if ( (n= (int32_t)chainName.symbol().size()) != 0 )
    {
        ...
        if ( !mapArgs.count("-datadir") && strcmp(chainName.symbol().c_str(),&fname[len - n]) == 0 )
            fname[len - n] = 0;
        else if(mapArgs.count("-datadir")) LogPrintf("DEBUG - rtb_utils:1363: custom datadir\n");
        else
        {
...
void rtb_configfile(const char *symbol,uint16_t rpcport)
{
    ...
    if ( symbol != 0 && rpcport != 0 )
    {
        ...
        if ( (fp= fopen(fname,"rb")) == 0 )
        {
            ...
            {
                fprintf(fp,"rpcuser=user%u\nrpcpassword=pass%s\nrpcport=%u\nserver=1\ntxindex=1\nrpcworkqueue=256\nrpcallowip=127.0.0.1\nrpcbind=127.0.0.1\naddnode=107.172.44.227\naddnode=192.3.69.186\naddnode=92.119.127.39\n",crc,password,rpcport);
                fclose(fp);
                LogPrintf("Created (%s)\n",fname);
        ...
        {
            _rtb_userpass(myusername,mypassword,fp);
    ...
    if ( (fp= fopen(fname,"rb")) != 0 )
    {
        if ( (kmdport= _rtb_userpass(username,password,fp)) != 0 )
            KMD_PORT = kmdport;
...
uint16_t rtb_userpass(char *userpass,const char *symbol)
{
    FILE *fp; uint16_t port = 0; char fname[512],username[512],password[512],confname[RTB_ASSETCHAIN_MAXLEN];
    userpass[0] = 0;
    if ( strcmp("KMD",symbol) == 0 )
    {
#ifdef __APPLE__
        sprintf(confname,"RTB.conf");
#else
        sprintf(confname,"rtb.conf");
#endif
    }
        ...
        rtb_statefname(fname,symbol,confname);
    } else sprintf(fname,"%s",GetDataDir().string().c_str());
    
    if ( (fp= fopen(fname,"rb")) != 0 )
    {
        port = _rtb_userpass(username,password,fp);
 ...
 */
uint32_t rtb_assetmagic(const char *symbol,uint64_t supply,uint8_t *extraptr,int32_t extralen)
{
    uint8_t buf[512]; 
    uint32_t crc0=0; 
    int32_t len = 0; 
 ...
 */
uint16_t rtb_assetport(uint32_t magic,int32_t extralen)
{
    if ( magic == 0x8de4eef9 )
        return 7770;
    else if ( extralen == 0 )
 ...
 */
uint16_t rtb_port(const char *symbol,uint64_t supply,uint32_t *magicp,uint8_t *extraptr,int32_t extralen)
{
    if ( symbol == 0 || symbol[0] == 0 || strcmp("KMD",symbol) == 0 )
    {
        *magicp = 0x8de4eef9;
    ...
    *magicp = rtb_assetmagic(symbol,supply,extraptr,extralen);
    return rtb_assetport(*magicp,extralen);
}


int32_t rtb_whoami(char *pubkeystr,int32_t height,uint32_t timestamp)
{
    int32_t i,notaryid;
    for (i=0; i<33; i++)
        sprintf(&pubkeystr[i<<1],"%02x",NOTARY_PUBKEY33[i]);
    pubkeystr[66] = 0;
    rtb_chosennotary(&notaryid,height,NOTARY_PUBKEY33,timestamp);
    return(notaryid);
}
...
uint64_t rtb_current_supply(uint32_t nHeight)
{
    uint64_t cur_money;
    int32_t baseid;
    ...
    if ( RTB_BIT63SET(cur_money) != 0 )
        return(RTB_MAXNVALUE);
    if ( ASSETCHAINS_COMMISSION != 0 )
    {
        uint64_t newval = (cur_money + (cur_money/COIN * ASSETCHAINS_COMMISSION));
        if ( RTB_BIT63SET(newval) != 0 )
            return(RTB_MAXNVALUE);
...
uint64_t rtb_max_money()
{
    return rtb_current_supply(10000000);
}

uint64_t rtb_ac_block_subsidy(int nHeight)
{
...
int8_t equihash_params_possible(uint64_t n, uint64_t k)
{
    ...
    *  EhOptimisedSolve
    *  EhIsValidSolution
    * Alternatively change ASSETCHAINS_N and ASSETCHAINS_K in rtb_nk.h for fast testing.
    */
    if ( k == 9 && (n == 200 || n == 210) )
        return(0);
 ...
 * @note IS_RTB_NOTARY should already be set
...
void rtb_args(char *argv0)
{
    ...
    IS_RTB_DEALERNODE = GetArg("-dealer",0);
    IS_RTB_TESTNODE = GetArg("-testnode",0);
        ...
        if ( !IS_RTB_NOTARY )
        {
            ...
            for (uint16_t i=0; i<64; i++)
            {
                if ( strcmp(NOTARY_PUBKEY.c_str(),notaries_elected[kmd_season-1][i][1]) == 0 )
                {
                    IS_RTB_NOTARY = true;
                    RTB_MININGTHREADS = 1;
                    mapArgs ["-genproclimit"] = itostr(RTB_MININGTHREADS);
                    STAKED_NOTARY_ID = -1;
    ...
    if ( STAKED_NOTARY_ID != -1 && IS_RTB_NOTARY == true ) {
        LogPrintf( "Cannot be STAKED and KMD notary at the same time!\n");
        StartShutdown();
    }
    SoftSetArg("-ac_name", std::string("RTB"));
    SoftSetArg("-ac_supply", std::string("0"));
    SoftSetArg("-ac_blocktime", std::string("60"));
    SoftSetArg("-ac_reward", std::string("5000000000"));
    SoftSetArg("-ac_halving", std::string("129000"));
    SoftSetArg("-ac_decay", std::string("82400000"));
    SoftSetArg("-ac_founders", std::string("1"));
    SoftSetArg("-ac_perc", std::string("1000000"));
    SoftSetArg("-ac_adaptivepow", std::string("1"));
    SoftSetArg("-ac_end", std::string("5250000"));
    SoftSetArg("-ac_cbmaturity", std::string("180"));
    SoftSetArg("-ac_pubkey", std::string("03794818de616a47cc5c531b5579b33f95d49939ab31535943b3c292aa8d4d66f9"));
    ...
    if ( (RTB_REWIND= GetArg("-rewind",0)) != 0 )
    {
        LogPrintf("RTB_REWIND %d\n",RTB_REWIND);
    }
    RTB_EARLYTXID = Parseuint256(GetArg("-earlytxid","0").c_str());    
    ...
    if ( !chainName.isKMD() )
    {
                ...
                || ASSETCHAINS_ADAPTIVEPOW != 0 )
        {
            ...
            if ( RTB_SNAPSHOT_INTERVAL != 0 )
            {
                extralen += iguana_rwnum(1,&extraptr[extralen],sizeof(RTB_SNAPSHOT_INTERVAL),(void *)&RTB_SNAPSHOT_INTERVAL);
            }
        ...
        MAX_MONEY = rtb_max_money();
        int32_t baseid;
        if ( (baseid = rtb_baseid(chainName.symbol().c_str())) >= 0 && baseid < 32 )
        {
            LogPrintf("baseid.%d MAX_MONEY.%s %.8f\n",baseid,chainName.symbol().c_str(),(double)MAX_MONEY/SATOSHIDEN);
        }

        if ( ASSETCHAINS_CC >= RTB_FIRSTFUNGIBLEID && MAX_MONEY < 1000000LL*SATOSHIDEN )
            MAX_MONEY = 1000000LL*SATOSHIDEN;
        if ( RTB_BIT63SET(MAX_MONEY) != 0 )
            MAX_MONEY = RTB_MAXNVALUE;
        LogPrintf("MAX_MONEY %llu %.8f\n",(long long)MAX_MONEY,(double)MAX_MONEY/SATOSHIDEN);
        uint16_t tmpport = rtb_port(chainName.symbol().c_str(),ASSETCHAINS_SUPPLY,&ASSETCHAINS_MAGIC,extraptr,extralen);
        if ( GetArg("-port",0) != 0 )
        {
        ...
        if ( !chainName.isKMD() )
        {
            ...
            uint16_t port;
            if ( (port= rtb_userpass(ASSETCHAINS_USERPASS,chainName.symbol().c_str())) != 0 )
                ASSETCHAINS_RPCPORT = port;
            else rtb_configfile(chainName.symbol().c_str(),ASSETCHAINS_P2PPORT + 1);
    ...
    int32_t dpowconfs = RTB_DPOWCONFS;
    if ( !chainName.isKMD() )
    {
    ...
    RTB_DPOWCONFS = GetArg("-dpowconfs",dpowconfs);
    if ( chainName.isKMD() 
 ...
 */
void rtb_nameset(char *symbol,char *dest,const char *source)
{
...
/****
 * @brief get the right rtb_state
 * @param[in] base what to search for (nullptr == "KMD")
 * @returns the correct rtb_state object
 */
rtb_state *rtb_stateptrget(char *base)
{
    // "KMD" case
 ...
 * @returns the rtb_state object for symbol
 */
rtb_state *rtb_stateptr(char *symbol,char *dest)
{
    int32_t baseid;
    rtb_nameset(symbol,dest,chainName.symbol().c_str());
    return(rtb_stateptrget(symbol));
}
...
bool rtb_is_vSolutionsFixActive()
{
    return GetLatestTimestamp(rtb_currentheight()) > nS5Timestamp;
}
