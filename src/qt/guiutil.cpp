...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent, bool allowZAddresses)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a RTB address (e.g. %1)").arg(
...
{
    // return if URI is not valid or is no rtb: URI
    if(!uri.isValid() || uri.scheme() != QString("rtb"))
        return false;
...
bool parseKomodoURI(QString uri, SendCoinsRecipient *out)
{
    // Convert rtb:// to rtb:
    //
    //    Cannot handle this later, because rtb:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("rtb://", Qt::CaseInsensitive))
    {
        uri.replace(0, 10, "rtb:");
    }
...
{
    QString ret = QString("rtb:%1").arg(info.address);
    int paramCount = 0;
...
bool openKomodoConf()
{
    ...
    /* Open rtb.conf with the associated application */
...
//    std::string chain = ChainNameFromCommandLine();
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "RTB.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "RTB (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("RTB (%s).lnk", chain);
}
...
fs::path static GetAutostartFilePath()
{
    ...
    CBaseChainParams::Network chain = NetworkIdFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "rtb.desktop";
    return GetAutostartDir() / strprintf("rtb-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        //std::string chain = ChainNameFromCommandLine();
        CBaseChainParams::Network chain = NetworkIdFromCommandLine();
        // Write a rtb.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=RTB\n";
        else
            optionFile << strprintf("Name=RTB (%s)\n", chain);
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    ...
    // loop through the list of startup items and try to find the rtb app
...
bool GetStartOnSystemStartup()
{
    CFURLRef rtbAppUrl = CFBundleCopyBundleURL(CFBundleGetMainBundle());
    if (rtbAppUrl == nullptr) {
        return false;
    }
    
    LSSharedFileListRef loginItems = LSSharedFileListCreate(nullptr, kLSSharedFileListSessionLoginItems, nullptr);
    LSSharedFileListItemRef foundItem = findStartupItemInList(loginItems, rtbAppUrl);
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add rtb app to startup item list
        LSSharedFileListInsertItemURL(loginItems, kLSSharedFileListItemBeforeFirst, nullptr, nullptr, rtbAppUrl, nullptr, nullptr);
    }
