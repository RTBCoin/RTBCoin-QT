...
{
    return QCoreApplication::translate("rtb-core", psz).toStdString();
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. rtb_de.qm (shortcut "de" needs to be defined in rtb.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. rtb_de_DE.qm (shortcut "de_DE" needs to be defined in rtb.qrc)
...
/** Class encapsulating RTB Core startup and shutdown.
...
public:
    ...
    /// Get window identifier of QMainWindow (RTBCoinGUI)
...
static const char* qt_argv = "rtb-qt";
    ...
    returnValue(0)
{
    ...
    platformName = GetArg("-uiplatform", RTBCoinGUI::DEFAULT_UIPLATFORM);
...
{
    window = new RTBCoinGUI(platformStyle, networkStyle, 0);
...
void KomodoApplication::initializeResult(bool success)
{
    ...
    if(success)
    {
        ...
        if (!vpwallets.empty())
        {
            ...
            window->addWallet(RTBCoinGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(RTBCoinGUI::DEFAULT_WALLET);
...
{
    QMessageBox::critical(0, "Runaway exception", RTBCoinGUI::tr("A fatal error occurred. RTB can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(EXIT_FAILURE);
}
...
int main(int argc, char *argv[])
{
    ...
    /// 6. Determine availability of data directory and parse rtb.conf
    ...
    try {
//        ReadConfigFile(GetArg("-conf", RTB_CONF_FILENAME));
...
//        SelectParams(ChainNameFromCommandLine());
    ...
    // rtb: links repeatedly have their payment requests routed to this process:
