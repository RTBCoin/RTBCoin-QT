...
#include "rtb_utils.h" // rtb_stateptrget
#include "rtb_bitcoind.h" // rtb_checkcommission
 ...
 */
bool rtb_checkvout(int32_t vout,int32_t k,int32_t indallvouts)
{
 ...
 */
int32_t rtb_bannedset(int32_t *indallvoutsp,uint256 *array,int32_t max)
{
    if ( sizeof(banned_txids)/sizeof(*banned_txids) > max )
    {
        LogPrintf("rtb_bannedset: buffer too small %d vs %d\n",(int32_t)(sizeof(banned_txids)/sizeof(*banned_txids)),max);
        StartShutdown();
    }
    int32_t i;
 ...
 */
int32_t rtb_check_deposit(int32_t height,const CBlock& block)
{
            ...
            for (int32_t j=0; j<n; j++) // for each vin
            {
                for (int32_t k=0; k<numbanned; k++) // for each banned txid
                {
                    if ( block.vtx[i].vin[j].prevout.hash == array[k] 
                            && rtb_checkvout(block.vtx[i].vin[j].prevout.n,k,indallvouts) )
                    {
         ...
         NetworkUpgradeActive(height, Params().GetConsensus(), Consensus::UPGRADE_SAPLING) )
    {
        ...
        if ( chainName.isKMD() )
        {
            ...
            else if ( block.nBits == RTB_MINDIFF_NBITS && total > 0 ) // to deal with fee stealing
            {
                LogPrintf("notary mined ht.%d with extra %.8f\n",height,dstr(total));
                if ( height > RTB_NOTARIES_HEIGHT1 )
                    return(-1);
            }
            ...
            else if ( height > 814000 )
            {
                ...
                int32_t num;
                return(-1 * (rtb_electednotary(&num,script+1,height,0) >= 0) * (height > 1000000));
            }
        }
        else
        {
            ...
            {
                if ( (checktoshis= rtb_checkcommission((CBlock *)&block,height)) < 0 )
                {
...
void rtb_stateind_set(struct rtb_state *sp,uint32_t *inds,int32_t n,uint8_t *filedata,long datalen,const char *symbol,const char *dest)
{
        ...
        for (lastfpos=fpos=prevpos100=i=0; i<n; i++)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    if ( doissue != 0 )
                    {
                        rtb_parsestatefiledata(sp,filedata,&lastfpos,datalen,symbol,dest);
                        count++;
                    }
                }
            }
...
/**
 * @brief Validate the index of the rtbstate file
 * 
 * @param[in] sp the rtb_state struct
 ...
 * @return -1 on error
 */
long rtb_stateind_validate(struct rtb_state *sp,const std::string& indfname,uint8_t *filedata,long datalen,
    ...
    if ( (inds= OS_fileptr(&fsize,indfname.c_str())) != 0 )
    {
        ...
        if ( (fsize % sizeof(uint32_t)) == 0 )
        {
            ...
            *prevpos100p = prevpos100;
            if ( sp != 0 )
                rtb_stateind_set(sp,(uint32_t *)inds,n,filedata,fpos,symbol,dest);
            free(inds);
            return fpos;
        } 
        else 
...
long rtb_indfile_update(FILE *indfp,uint32_t *prevpos100p,long lastfpos,long newfpos,uint8_t func,uint32_t *indcounterp)
{
    if ( indfp != 0 )
    {
...
/***
 * @brief read the rtbstate file
 * @param sp the rtb_state struct
 * @param fname the filename
 * @param symbol the chain symbol
 * @param dest the "parent" chain
 * @return true on success
 */
bool rtb_faststateinit(rtb_state *sp,const char *fname,char *symbol, const char *dest)
{
    ...
    if ( (filedata= OS_fileptr(&datalen,fname)) != 0 )
    {
        ...
        int32_t func;
        while (!ShutdownRequested() && (func= rtb_parsestatefiledata(sp,filedata,&fpos,datalen,symbol,dest)) >= 0)
        {
            lastfpos = rtb_indfile_update(indfp,&prevpos100,lastfpos,fpos,func,&indcounter);
        }
        if (ShutdownRequested()) { fclose(indfp); return false; }
        if ( indfp != nullptr )
        {
            fclose(indfp);
            if ( (fpos= rtb_stateind_validate(0,indfname,filedata,datalen,&prevpos100,&indcounter,symbol,dest)) < 0 )
                LogPrintf("unexpected rtbstate.ind validate failure %s datalen.%ld\n",indfname.c_str(),datalen);
            else 
...
//uint64_t rtb_interestsum(); // in wallet/rpcwallet.cpp

/***
 * @brief update wallet balance / interest
 * @note called only on KMD chain every 10 seconds ( see ThreadUpdateKomodoInternals() )
 */
void rtb_update_interest()
{
    static uint32_t lastinterest; // prevent needless rtb_interestsum calls
    if (rtb_chainactive_timestamp() > lastinterest)
    {
        rtb_interestsum();
        lastinterest = rtb_chainactive_timestamp();
    }
