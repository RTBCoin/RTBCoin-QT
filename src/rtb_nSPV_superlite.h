...
#ifndef RTB_NSPVSUPERLITE_H
#define RTB_NSPVSUPERLITE_H
...
// rtb_nSPVresp is called from async message processing

void rtb_nSPVresp(CNode *pfrom,std::vector<uint8_t> response) // received a response
{
...
// rtb_nSPV from main polling loop (really this belongs in its own file, but it is so small, it ended up here)

void rtb_nSPV(CNode *pto) // polling loop from SendMessages
{
...
UniValue NSPV_getinfo_json(struct NSPV_inforesp *ptr)
{
    ...
    result.push_back(Pair("result","success"));
    result.push_back(Pair("nSPV",RTB_NSPV==-1?"disabled":(RTB_NSPV_SUPERLITE?"superlite":"fullnode")));
