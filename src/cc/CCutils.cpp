...
{
    if ( RTB_NSPV_SUPERLITE ) return ((uint32_t)NSPV_blocktime(height));
    return(rtb_heightstamp(height));
}
...
int64_t CCduration(int32_t &numblocks,uint256 txid)
{
    ...
    else if ( (pindex= rtb_getblockindex(hashBlock)) == 0 || (txtime= pindex->nTime) == 0 || (txheight= pindex->nHeight) <= 0 )
    {
...
int32_t rtb_get_current_height()
{
    if ( RTB_NSPV_SUPERLITE )
    {
...
bool rtb_txnotarizedconfirmed(uint256 txid)
{
    ...
    if ( RTB_NSPV_SUPERLITE )
    {
        ...
        {
            LogPrintf("rtb_txnotarizedconfirmed cant find txid %s\n",txid.ToString().c_str());
            return(0);
        }
        else if (txheight<=0)
        {
            LogPrintf("rtb_txnotarizedconfirmed no txheight.%d for txid %s\n",txheight,txid.ToString().c_str());
            return(0);
        }
        else if (txheight>currentheight)
        {
            LogPrintf("rtb_txnotarizedconfirmed backwards heights for txid %s hts.(%d %d)\n",txid.ToString().c_str(),txheight,currentheight);
            return(0);
        }
    ...
    else
    {
        ...
        {
            LogPrintf("rtb_txnotarizedconfirmed no hashBlock for txid %s\n",txid.ToString().c_str());
            return(0);
        }
        else if ( (pindex= rtb_blockindex(hashBlock)) == 0 || (txheight= pindex->nHeight) <= 0 )
        {
            LogPrintf("rtb_txnotarizedconfirmed no txheight.%d %p for txid %s\n",txheight,pindex,txid.ToString().c_str());
            return(0);
        }
    ...
    if ((sp= rtb_stateptr(symbol,dest)) != 0 && (notarized=sp->LastNotarizedHeight()) > 0 && txheight > sp->LastNotarizedHeight())  notarized=0;            
#ifdef TESTMODE           
    notarized=0;
#endif //TESTMODE
...
{
    if ( RTB_CONNECTING < 0 ) // always comes back with > 0 for final confirmation
        return true;

    if ( ASSETCHAINS_CC == 0 || (RTB_CONNECTING & ~(1<<30)) < RTB_CCACTIVATE )
...
bool CClib_Dispatch(const CC *cond,Eval *eval,std::vector<uint8_t> paramsNull,const CTransaction &txTo,unsigned int nIn)
{
    ...
    int32_t height = RTB_CONNECTING;
    if ( ASSETCHAINS_CC == 0 || (height & ~(1<<30)) < RTB_CCACTIVATE )
        return eval->Invalid("CC are disabled or not active yet");
    if ( (RTB_CONNECTING & (1<<30)) != 0 )
    {
        height &= ((1<<30) - 1);
    }
