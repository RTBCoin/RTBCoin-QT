...
{
    CMutableTransaction mtx = CreateNewContextualCMutableTransaction(Params().GetConsensus(), rtb_nextheight()), burntx = CreateNewContextualCMutableTransaction(Params().GetConsensus(), rtb_nextheight());
    ...
    if (RTB_EARLYTXID!=zeroid && bindtxid!=RTB_EARLYTXID)
    {
        CCerror = strprintf("invalid import gateway. On this chain only valid import gateway is %s",RTB_EARLYTXID.GetHex());
        LOGSTREAM("importgateway",CCLOG_INFO, stream << CCerror << std::endl);
        return("");
    }
    cp = CCinit(&C,EVAL_IMPORTGATEWAY);
...
std::string ImportGatewayPartialSign(uint64_t txfee,uint256 lasttxid,std::string refcoin, std::string hex)
{
    ...
    if (funcid=='W')
    {
        ...
        else if (RTB_EARLYTXID!=zeroid && bindtxid!=RTB_EARLYTXID)
        {
            CCerror = strprintf("invalid import gateway. On this chain only valid import gateway is %s",RTB_EARLYTXID.GetHex());
            LOGSTREAM("importgateway",CCLOG_INFO, stream << CCerror << std::endl);
            return("");
