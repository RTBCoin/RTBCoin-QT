...
*/
extern int32_t rtb_get_current_height();
...
{
    CMutableTransaction mtx = CreateNewContextualCMutableTransaction(Params().GetConsensus(), rtb_nextheight()); CTransaction tx;
    CPubKey mypk,oraclespk; struct CCcontract_info *cp,C; std::string name,desc,format; int32_t numvouts; uint256 hashBlock;

    if (GetLatestTimestamp(rtb_currentheight())<PUBKEY_SPOOFING_FIX_ACTIVATION)
...
UniValue OracleRegister(const CPubKey& pk, int64_t txfee,uint256 oracletxid,int64_t datafee)
{
    ...
    {
        if (GetLatestTimestamp(rtb_currentheight())>PUBKEY_SPOOFING_FIX_ACTIVATION && AddMyOraclesFunds(cp,mtx,mypk,oracletxid)!=CC_MARKER_VALUE)
        ...
        if (GetLatestTimestamp(rtb_get_current_height())>PUBKEY_SPOOFING_FIX_ACTIVATION) mtx.vout.push_back(CTxOut(txfee,CScript() << ParseHex(HexStr(mypk)) << OP_CHECKSIG));
