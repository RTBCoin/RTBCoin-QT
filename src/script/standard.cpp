...
bool rtb_is_vSolutionsFixActive(); // didn't want to bring rtb headers here, it's a special case to bypass bad code in Solver() and ExtractDestination() 
...
bool ExtractDestination(const CScript& _scriptPubKey, CTxDestination& addressRet, bool returnPubKey)
{
    ...
    {
        if (vSolutions.size() > 1 && !rtb_is_vSolutionsFixActive()) // allow this temporarily before the HF; actually this is incorrect to use opdrop's pubkey as the address
        {
